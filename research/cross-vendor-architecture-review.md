...
This breaks a contract both memos assert. Maya's gate says "model mismatch/fallback fails closed"; Noah's says "no guessed IDs, automatic fallback or billable validation." A pin cannot be validated at spawn, because nothing validates it at spawn. But the news is better than it looks: **invalid-model validation is free.** A failed model resolves to zero cost. So the corrected contract is that validation happens at first turn, fails closed there, and costs nothing — and `--fallback-model` must be left permanently unset, because it exists precisely to do the silent substitution both memos correctly forbid.

This also answers Maya's uncertainty 2 and Noah's item 9. Resume compatibility needs no billable probe: pin the concrete id, diff it against `initialize`'s `models[].resolvedModel` set at relaunch, and if the first turn returns `model_not_found`, stop and surface. Never substitute.
...
`libghostty`'s full surface-plus-Metal API is **internal and explicitly unstable**. Ghostty's own C API overview: "The libghostty API is currently used primarily by the macOS app and is not yet stabilized for general-purpose embedding. The API may change significantly between releases." The repository states "we haven't tagged libghostty with a version yet." What *is* public is `libghostty-vt` — VT sequence parsing and terminal state only, no renderer, no input, no IME — and its author describes it as alpha, "not promising API stability," with surface/renderer/Swift-view libraries on the roadmap, not shipped. Ghostty is MIT and written in Zig, so embedding also imports a Zig toolchain into an Xcode build and notarization pipeline.
...
*tmux is not the survival substrate for a stream-json agent.* Both memos assert that Hive owns the composer and the vendor TUI is irrelevant, and simultaneously that tmux owns PTY survival with each `PaneUUID` mapped to a tmux target. Those cannot both be true. A headless `claude -p --input-format stream-json` process has no TUI and no PTY worth preserving; what must survive a UI crash is the *pipe owner*, which is the broker. The broker is a long-lived daemon; it can parent agent processes directly, and durability comes from the broker's database plus the vendor's own `--resume`/`thread/resume`. tmux remains genuinely necessary for exactly two things: hosting a real interactive TUI in the legacy compatibility driver, and giving the user a shell pane. This deletes an entire class of proposed gates ("tmux reattach must prove exact pane UUID→target mapping and no duplicate shell," "PTY resize storms absent") by deleting their subject.
...
One **Tenant Broker** per HiveUUID owns the canonical state, the event log, the worktree ledger, the capability epochs, approvals, and landing. It parents agent processes directly and owns their stdio pipes, so an agent survives any UI crash without tmux. It persists to its own SQLite under its own runtime directory.
...
The **daemon MCP endpoint gets authentication immediately**, ahead of any flagship work. Per-agent bearer tokens minted at spawn, bound to `(HiveUUID, agentName, capabilityEpoch)`, and a per-tenant unix socket rather than a shared TCP port. This is a bug fix, not a feature.

## De-risking prototypes

Five, each falsifiable in under a week, ordered by how much they would change the plan if they failed.

1. **Broker-owned stream-json agent, no tmux.** Spawn Claude Code and Codex headless from a long-lived parent, drive a full task with approvals, kill the UI, prove the agent continues, reattach, replay `pending_permission_requests`, and confirm no duplicate turn. If this fails, tmux returns as the survival substrate and the terminal surface returns with it.
2. **Transcript pane instead of a terminal.** Render one real agent session as structured AppKit views. Measure VoiceOver navigation, IME into the composer, find, and selection. If a transcript view cannot beat a terminal on comprehension, the renderer bet comes back.
...
6. No unauthenticated local process can call `hive_land`, `hive_kill`, or `hive_approve`. An agent's token is bound to its HiveUUID, name, and capability epoch; a revoked epoch is refused.
7. A hostile unsigned client cannot connect to a broker XPC listener. Capability FDs are not inherited by agent descendants.
8. Every ring-1 binding generation passes the identical conformance fixture: lifecycle, approve, deny, needs-user, steer, cancel-with-receipt, resume, read-only denial.
9. An invalid model pin fails closed at first turn, at zero cost, with no fallback and no substitution. `--fallback-model` is never passed.
10. Killing the UI leaves every agent running; reattaching replays pending approvals with no duplicate turn and no duplicate session. Killing the broker and restarting it recovers with a generation bump and monotonic event replay.
