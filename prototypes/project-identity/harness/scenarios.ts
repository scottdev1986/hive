...
function register(ctx: Ctx, dir: string, idempotencyKey = dir): string {
  ...
  if (result.status !== "RESOLVED") {
    throw new Error(
      `expected RESOLVED registering ${dir}, got ${result.status}`,
    );
  }
...
function explicitCreate(ctx: Ctx, resolution: Resolution): string {
  if (resolution.status !== "NEEDS_SETUP")
    throw new Error("explicitCreate needs NEEDS_SETUP");
  const bookmark = ctx.bookmarks.available
    ? ctx.bookmarks.create(resolution.key.canonicalPath)
    : null;
  return ctx.registry.create(resolution.key, resolution.evidence, bookmark)
    .hiveUuid;
}
...
function withTopology(
  ...
  body: (
    ctx: Ctx,
    t: ReturnType<typeof buildGitTopology>,
    check: Check,
  ) => void,
): ScenarioFn {
  return (check) => {
...
export function buildScenarios(bookmarks: BookmarkProvider): Scenario[] {
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.note(
        `realpath(${alias}) collapsed onto ${viaAlias.key.canonicalPath}`,
      );
    }),
  });
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.equal(
        nested.hiveUuid,
        uuid,
        "nested path must reach the root's Hive",
      );
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.assert(
        outer !== inner,
        "nested repository must not inherit the outer Hive",
      );
      const innerProbe = probeGit(t.innerRepo);
      check.note(
        `inner git-common-dir ${innerProbe.gitCommonDir} differs from outer`,
      );
    }),
  });
  ...
  scenarios.push({
    ...
    proves:
      "Step 4/5: a submodule is its own nearest project and does not share a landing lease.",
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.assert(
        result.hiveUuid !== mainUuid,
        "submodule must be a distinct Hive",
      );
      check.equal(
        result.key.kind,
        "git-submodule",
        "kind must be git-submodule",
      );
      check.assert(
        result.key.superprojectRoot === t.main,
        "superprojectRoot must name the superproject",
      );
  ...
  scenarios.push({
    id: "linked-worktree",
    title:
      "A user linked worktree is a distinct Hive that shares a repo family",
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.assert(
        wt.hiveUuid !== mainUuid,
        "linked worktree must be a distinct Hive",
      );
      check.equal(
        wt.key.kind,
        "git-linked-worktree",
        "kind must be git-linked-worktree",
      );
      const mainRecord = ctx.registry.findByUuid(mainUuid);
      check.equal(
        wt.key.repoFamilyKey,
        mainRecord?.repoFamilyKey ?? null,
        "must share the repo family",
      );
  ...
  scenarios.push({
    ...
    proves:
      "Step 5: separate clones are distinct and never share a landing lease.",
    run: withTopology(bookmarks, (ctx, t, check) => {
      const mainUuid = register(ctx, t.main);
      ...
      check.assert(
        clone.hiveUuid !== mainUuid,
        "clone must be a distinct Hive",
      );
      const mainRecord = ctx.registry.findByUuid(mainUuid);
      check.assert(
        clone.key.repoFamilyKey !== mainRecord?.repoFamilyKey,
        "clone must have its own repo family",
      );
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.equal(
        result.status,
        "REJECTED",
        "bare repository must be rejected",
      );
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.note(
        `unsanitized: GIT_DIR=${t.bare} makes rev-parse report ${hijacked}`,
      );
      check.assert(
        hijacked === t.bare,
        "the hijack must actually work, or this scenario proves nothing",
      );

      const sanitized = sanitizedGitEnv({ ...process.env, GIT_DIR: t.bare });
      check.assert(
        sanitized["GIT_DIR"] === undefined,
        "GIT_DIR must be stripped",
      );
      ...
      try {
        ...
        check.equal(
          probe.gitDir,
          join(t.main, ".git"),
          "sanitized discovery must ignore GIT_DIR",
        );
  ...
  scenarios.push({
    ...
    proves:
      "Gate 2: moved projects require confirmed rebind, and the HiveUUID survives.",
    run: withTopology(bookmarks, (ctx, t, check) => {
      const uuid = register(ctx, t.main);
      ...
      check.equal(
        result.status,
        "NEEDS_REBIND",
        "a renamed project must not silently attach",
      );
      ...
      check.equal(
        result.hiveUuid,
        uuid,
        "the HiveUUID must be preserved across the move",
      );
      check.note(
        `rename detected by evidence: ino unchanged at ${statSync(renamed).ino}`,
      );

      const rebound = ctx.registry.rebind(
        uuid,
        result.key,
        evidenceOf(renamed),
        ctx.bookmarks.create(renamed),
      );
      ...
      check.equal(
        after.status,
        "RESOLVED",
        "after an explicit rebind the project resolves",
      );
    }),
  });
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.equal(
        result.status,
        "NEEDS_REBIND",
        "moved project must not silently attach",
      );
      if (result.status !== "NEEDS_REBIND") return;
      check.equal(result.hiveUuid, uuid, "HiveUUID preserved");
      check.equal(
        result.confirmedCanonicalPath,
        t.main,
        "the stale confirmed path is reported",
      );
    }),
  });
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      if (ctx.bookmarks.available && record?.bookmark) {
        ...
        check.note(
          `bookmark resolved to ${resolved?.path} (isStale=${resolved?.isStale})`,
        );
        check.assert(
          resolved?.path === t.main,
          "measured: the bookmark abandons the moved dir for the impostor",
        );
        check.note(
          `the real project is alive at ${moved}, yet the bookmark points at ${t.main}`,
        );
        check.note(
      ...
      check.equal(
        atOldPath.status,
        "NEEDS_SETUP",
        "the impostor must not resolve to the old Hive",
      );
      if (atOldPath.status !== "NEEDS_SETUP") return;
      check.equal(
        atOldPath.reason,
        "TOMBSTONED_PATH",
        "the old path must be tombstoned",
      );
      check.equal(
        atOldPath.formerHiveUuid,
        uuid,
        "the tombstone names the evicted Hive",
      );
      check.note(
        `refusal came from evidence: ino ${record?.evidence.ino} != ${statSync(t.main).ino}`,
      );
      ...
      check.equal(
        atNewPath.status,
        "NEEDS_REBIND",
        "the real project is still findable at its new path",
      );
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.equal(
        result.status,
        "NEEDS_SETUP",
        "recreated path must not inherit",
      );
      if (result.status !== "NEEDS_SETUP") return;
      check.equal(
        result.reason,
        "TOMBSTONED_PATH",
        "the old identity is tombstoned",
      );
      check.equal(
        result.formerHiveUuid,
        uuid,
        "the tombstone names the old Hive",
      );
      ...
      check.equal(
        old?.state,
        "NEEDS_REBIND",
        "the evicted Hive survives in NEEDS_REBIND, not silently deleted",
      );
    }),
  });
  ...
  scenarios.push({
    ...
    run: withTopology(bookmarks, (ctx, _t, check) => {
      ...
      check.equal(
        child.status,
        "AMBIGUOUS_PLAIN_ANCESTOR",
        "a registered plain ancestor forces a choice",
      );
      if (child.status !== "AMBIGUOUS_PLAIN_ANCESTOR") return;
      check.equal(child.ancestorHiveUuid, uuid, "the ancestor is named");
      check.note(
        `child ${child.key.canonicalPath} did not silently attach to ${child.ancestorPath}`,
      );
    }),
  });
  ...
  scenarios.push({
    id: "use-parent",
    title:
      "Use Parent is ephemeral: legal before the child is registered, refused after",
    proves:
      "Step 4: 'Use Parent' is an ephemeral override only before registration.",
    run: withTopology(bookmarks, (ctx, t, check) => {
      const outer = register(ctx, t.main);

      const before = resolveProject(t.innerRepo, {
        ...ctx.options,
        useParent: true,
      });
      check.equal(
        before.status,
        "RESOLVED",
        "Use Parent resolves to the parent before registration",
      );
      ...
      const after = resolveProject(t.innerRepo, {
        ...ctx.options,
        useParent: true,
      });
      check.equal(
        after.status,
        "REJECTED",
        "Use Parent is refused once the child exists",
      );
  ...
  scenarios.push({
    ...
    title:
      "Managed-worktree ownership comes from the authenticated ledger, never from a repo file",
    proves:
      "Step 2: a repository file cannot assert managed-worktree ownership.",
    run: withTopology(bookmarks, (ctx, t, check) => {
      ...
      check.assert(
        unclaimed.hiveUuid !== owner,
        "the planted file did NOT grant ownership",
      );
      check.note(
        `.hive/owner.json claimed ${owner}; resolver assigned ${unclaimed.hiveUuid}`,
      );
      ...
      check.equal(
        claimed.hiveUuid,
        owner,
        "the ledger routes it to the owning Hive",
      );
      check.equal(
        claimed.key.kind,
        "managed-worktree",
        "kind reflects ledger ownership",
      );
  ...
  scenarios.push({
    ...
    proves:
      "Gate 3: resolveOrCreate runs under a unique constraint and creation lease.",
    run: withTopology(bookmarks, (ctx, t, check) => {
  ...
  scenarios.push({
    ...
    run: (check) => {
      if (!bookmarks.available)
        throw new SkipScenario("swiftc unavailable; no Foundation bookmarks");
      const root = tempRoot();
      try {
        const a = join(root, "A");
        const b = join(root, "B");
        ...
        check.equal(
          quiet?.isStale,
          false,
          "an untouched directory is not stale",
        );
        ...
        check.equal(
          afterMove?.path,
          b,
          "with A vacant, the bookmark follows the move to B",
        );
        check.equal(afterMove?.isStale, true, "and reports isStale");
        check.note(
          `after move: path=${afterMove?.path} isStale=${afterMove?.isStale}`,
        );
        ...
        check.equal(
          afterImpostor?.path,
          a,
          "once A is repopulated the bookmark abandons B for A",
        );
        check.note(
          `after impostor: path=${afterImpostor?.path} isStale=${afterImpostor?.isStale}`,
        );
        check.assert(
          statSync(a).ino !== statSync(b).ino,
          "A and B are different inodes",
        );
        check.note(
          `B still exists (ino ${statSync(b).ino}); bookmark chose A (ino ${statSync(a).ino})`,
        );
        ...
        check.equal(
          afterDelete?.path,
          a,
          "with the real directory deleted the bookmark still names A",
        );
      } finally {
        rmSync(root, { recursive: true, force: true });
      }
    ...
    run: (check) => {
      ...
      try {
        ...
        check.equal(
          seen.size,
          500,
          "500 create/delete cycles produced 500 distinct inodes",
        );
  ...
  scenarios.push({
    ...
    run: onDiskImage(false, bookmarks, (ctx, mount, check) => {
      ...
      check.equal(
        ...
        false,
        "volume must be case-insensitive",
      );
      check.note(
        `volume case behavior: caseSensitive=${volume.caseSensitive} via ${volume.caseProvenance}`,
      );
      ...
      check.equal(
        viaLower.status,
        "RESOLVED",
        "the lowercase spelling resolves",
      );
      if (viaLower.status !== "RESOLVED") return;
      check.equal(viaLower.hiveUuid, uuid, "both spellings are one Hive");
      check.note(
        `realpath rewrote 'project' to on-disk spelling ${viaLower.key.canonicalPath}`,
      );
    }),
  });
  ...
  scenarios.push({
    ...
    proves:
      "Step 1: identity-key folding must never merge two real directories.",
  ...
  scenarios.push({
    ...
    title:
      "A cross-volume move is a copy and must not inherit the old identity",
    proves:
      "Evidence-based move detection degrades safely: a new inode is a new project.",
    run: onDiskImage(false, bookmarks, (ctx, mount, check) => {
      ...
      check.assert(
        beforeDev !== afterDev,
        "the copy landed on a different volume",
      );
      check.note(
        `dev ${beforeDev} -> ${afterDev}; a cross-volume move cannot preserve the inode`,
      );
      ...
      check.equal(
        result.status,
        "NEEDS_SETUP",
        "the copy is a new project, not the old Hive",
      );
      if (result.status !== "NEEDS_SETUP") return;
      check.equal(
        result.reason,
        "NEW_PROJECT",
        "no evidence links it to the old Hive",
      );
      check.note(
        `the old Hive ${uuid} is not silently transplanted; it remains registered at its old path`,
      );
    }),
  });
  ...
  return (check) => {
    ...
    try {
      ...
      try {
        ...
        if (error instanceof DiskImageUnavailable)
          throw new SkipScenario(error.message);
        throw error;
      }
