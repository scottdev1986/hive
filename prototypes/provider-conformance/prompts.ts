...
export const PROMPTS: Record<Scenario, ScenarioPrompt> = {
  lifecycle: {
    purpose:
      "Produce one minimal successful turn so lifecycle boundaries and the effective model are observable.",
    text: () => "Reply with exactly HIVE_LIFECYCLE_OK. Do not use tools.",
    realTask: true,
  },
  approve: {
    purpose:
      "Force one correlated write approval, approve it, and verify the approved side effect.",
    text: (markerPath) =>
  ...
  deny: {
    purpose:
      "Force one correlated write approval, deny it, and verify that the side effect did not occur.",
    text: (markerPath) =>
  ...
  "needs-user": {
    purpose:
      "Force one native clarification request and answer it through the provider control protocol.",
    text: () =>
  ...
  steer: {
    purpose:
      "Keep a turn active long enough to inject a direction change and verify the accepted steer affects the final response.",
    text: () =>
  ...
  cancel: {
    purpose:
      "Keep a turn active long enough to cancel it and require both command acknowledgement and a terminal interrupted state.",
    text: () =>
  ...
  resume: {
    purpose:
      "Create durable conversation state, restart the provider process, resume by the recorded session id, and verify prior context.",
    text: () =>
      "Remember the token HIVE_RESUME_ANCHOR and reply with exactly HIVE_RESUME_STORED.",
    realTask: true,
  },
  "invalid-model": {
    purpose:
      "Validation-only sentinel. It is not a user task and must be rejected before any real task is accepted.",
    text: () =>
      "Reply with exactly HIVE_PROVIDER_VALIDATION_OK. Do not use tools.",
    realTask: false,
  },
  "read-only": {
    purpose:
      "Attempt one write under a non-interactive read-only policy and verify policy denial plus absence of the marker.",
    text: (markerPath) =>
  ...
  "dual-client": {
    purpose:
      "Attach the Codex TUI and a second protocol client to one durable thread, accept a steer, inject model-visible history, and verify the TUI receives the shared result.",
    text: () =>
      "Use the shell tool to run /bin/sleep 8. After it finishes, reply with exactly HIVE_ORIGINAL_DIRECTION.",
    realTask: true,
  },
...
export const DUAL_CLIENT_INJECT_TEXT =
  "Remember the exact token HIVE_INJECT_SEEN.";
