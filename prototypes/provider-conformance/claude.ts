...
const CLAUDE_INPUT_DOCS =
  "https://code.claude.com/docs/en/agent-sdk/user-input";
const CLAUDE_STREAM_DOCS =
  "https://code.claude.com/docs/en/agent-sdk/streaming-vs-single-mode";
const DRIVEN_REVIEW =
  "raw/reviews/cross-vendor-architecture-review.md#appendix-reproducing-the-claude-code-observations";
...
function usageFromResult(message: JsonObject): {
  input: number;
  output: number;
} {
  const usage =
...
function handleFrame(message: JsonObject, session: ClaudeSession): void {
  ...
  if (type === "system" && subtype === "init") {
    ...
    if (permissionMode !== undefined) {
      session.recorder.add({
        ...
        sessionId,
        status:
          permissionMode === "dontAsk" && session.scenario === "read-only"
            ? "read-only"
            : permissionMode,
      });
    }
  ...
  if (type === "control_request") {
    ...
    const requestId = stringValue(
      message.request_id,
      "Claude control request id",
    );
    if (request.subtype !== "can_use_tool") {
      sendControlResponse(session.process, requestId, {
        behavior: "deny",
        message: "Unsupported fixture control request",
      });
      session.recorder.add({
        type: "diagnostic",
        requestId,
        text: `Unsupported Claude control request: ${String(request.subtype)}`,
      });
      return;
    }
    ...
    for (const block of contentBlocks(message)) {
      ...
      if (block.type === "tool_use") {
        session.recorder.add({
          ...
          metadata:
            typeof block.id === "string" ? { toolUseId: block.id } : undefined,
        });
      }
    }
    return;
    ...
    for (const block of contentBlocks(message)) {
      ...
      const denied =
        ...
        (typeof block.content === "string" &&
          /denied|not allowed|permission/i.test(block.content));
      session.recorder.add({
  ...
  if (type === "system" && subtype === "permission_denied") {
    session.recorder.add({
      ...
      turnId: `turn-${session.currentTurn}`,
      tool:
        typeof message.tool_name === "string" ? message.tool_name : "unknown",
  ...
  if (type === "result") {
    ...
    const text =
      typeof message.result === "string" ? message.result : session.text;
  ...
  return session.process.waitFor((message) => {
    ...
    return (
      ...
      (typeof message.result === "string" &&
        message.result.includes(expectedText))
    );
  });
}
...
async function initializeProbe(
  ...
  try {
    ...
    const selectedModel =
      ...
      stringValue(model.resolvedModel ?? model.value, "Claude resolved model");
    return {
      selectedModel,
      provenance: [
...
async function runScenario(
  ...
  const realTaskStarted = scenario !== "invalid-model";
  ...
  try {
    if (scenario === "resume") {
      ...
      const durableId = stringValue(
        session.sessionId,
        "Claude durable session id",
      );
      ...
      await waitForResult(
        session,
        scenario === "steer" ? "HIVE_STEERED_OK" : undefined,
      );
      if (scenario === "invalid-model") {
        if (
          session.effectiveModel !== undefined &&
          session.effectiveModel !== context.invalidModel
        ) {
          recorder.add({
