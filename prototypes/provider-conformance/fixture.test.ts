...
import { codexRootEligible, compactReport } from "./promote";
...
describe("provider-neutral assertions", () => {
  ...
  test("requires an acknowledged cancel and terminal interrupted state", () => {
    ...
    expect(
      missingReceipt.assertions.find((item) => item.id === "cancel-receipt")
        ?.pass,
    ).toBe(false);
    expect(
      evaluate(
    ...
    expect(
      evaluate(
        run("resume", [
          ...
          event("turn.completed", {
            text: "HIVE_RESUME_OK:HIVE_RESUME_ANCHOR",
          }),
        ]),
      ).outcome,
    ).toBe("pass");
    ...
    expect(
      evaluate({
        ...
        events: result.events.filter(
          (entry) => entry.type !== "client.observed",
        ),
      }).outcome,
    ).toBe("fail");
  });
...
describe("billing and probe safety", () => {
  test("round-trips JSON-RPC frames over the dual-client WebSocket transport", async () => {
    ...
    const transport = await JsonWebSocket.connect({
      ...
      capturePath: join(
        import.meta.dir,
        "runs",
        "offline-websocket.frames.jsonl",
      ),
      timeoutMs: 2_000,
    });
    try {
  ...
  test("refuses live billable and cost-unknown scenarios without the explicit flag", () => {
    ...
    expect(requiresBillableAuthorization(options)).toEqual([
      "codex/invalid-model:unknown",
    ]);
