...
One recipient is special: the orchestrator lives in the user's own terminal, not a tmux pane, so there is nothing to inject into. Instead of registering it as a fake agent (a row with a tmux session that doesn't exist — the first shipped shape, which made `hive_send to: "orchestrator"` fail with "recipient not found" and silently orphaned every completion report), "orchestrator" is a reserved name: never spawnable, always a valid destination, always queue-only. Agents report with `hive_send to "orchestrator"`; the orchestrator drains its own inbox with `hive_inbox`. The cost is that nothing *wakes* the orchestrator — a report waits until it next looks, which its brief tells it to do whenever it checks status.
