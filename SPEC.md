...
The split that works: **the orchestrator classifies; a table resolves.** The orchestrator's judgment call is "this subtask is `deep` / `standard` / `cheap`" — a thing LLMs are genuinely good at and that never goes stale. A routing table maps tiers to concrete invocations — `deep` → Claude Opus, high effort; `standard`/`cheap` → Codex at medium/low effort. Mechanically this is just spawn flags: `claude --model …`, `codex -c model_reasoning_effort=…`.

A corollary the first field test taught us: **routes must not assume account entitlements.** Naming a concrete Codex model in the defaults killed every Codex spawn on a ChatGPT-plan account ("model not supported"). Default routes therefore say model `"default"` — pass no model flag and let the CLI use whatever the account actually has, which is supported by definition; effort flags still steer cost per tier. Pinning exact model IDs is opt-in via `routing.toml`, where the user knows their own plan.
