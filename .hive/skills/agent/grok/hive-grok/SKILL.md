...
So: **never read "the command exited 0" as "the work happened."** If a tool comes back cancelled and you never saw an approval request, stop. That is a Hive launch-configuration bug — report it to queen naming the tool, and do not grind a retry loop against a permission that cannot be granted mid-turn.
