...
Resume order: read the handoff and blueprint; verify the documentation and source research commits; check once whether `docs/research/model-routing-and-token-efficiency.md` has landed and link it if present (otherwise leave independently pending); scope and land Phase 0 before any Swift flagship work; then fix build/project handshake; run prototypes; build Supervisor/registry/brokers; migrate legacy `HIVE_HOME`/tmux transactionally; build the AppKit transcript workspace; add SwiftTerm only for shell/legacy panes.
