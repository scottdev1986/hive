...
} from "../../src/daemon/lifecycle/startup-announcement";
...
if (import.meta.main) {
  ...
  if (args[0] === "--memory") {
    const devHome = args[1];
    if (devHome === undefined || args.length !== 2) {
    ...
    const [logPath, expectedBinary, repoRoot, daemonPidRaw, devHome] = args;
