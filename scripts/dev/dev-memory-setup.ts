// `make run` dev-home memory sharing. The dev daemon runs on its own HIVE_HOME so RUNTIME state stays isolated; MEMORY state must match installed hive. Links four paths from $HOME/.hive into the dev home: memory/ global wiki projects/ per-project episodic stores project-registry.json repo→hiveUuid — without it, dev mints a different uuid and episodic stores diverge models/ embedding model cache Everything else stays per-home. WHITELIST only — runtime is never linked. Rules (test/scripts/dev-memory-setup.test.ts): - Create real memory/, projects/, models/ and empty project-registry.json when absent so a dev-first machine shares identity with a later install. - Never delete or merge a real directory/file already at a dev path; non-empty pre-existing state gets a warning and is left alone. - Refresh stale symlinks; leave correct ones alone. Clean-safety: entries are symlinks; `make clean` rm -rf unlinks without following, so real ~/.hive is unreachable.
import {
  lstat,
  mkdir,
  readlink,
