...
if [[ ! -f "$MAC_PLIST" ]]; then
  echo "GhosttyKit artifact missing: $MAC_PLIST (run scripts/native/build-ghosttykit.sh)" >&2
  exit 1
fi
...
for arch in arm64 x86_64; do
  ...
  if [[ ! -f "$VT_LIB" ]]; then
    echo "lib-vt artifact missing: $VT_LIB (run scripts/native/build-ghosttykit.sh)" >&2
    exit 1
  fi
  ...
  # Both architecture authoring runs are mandatory. On Apple Silicon the x86_64 invocation proves Rosetta is present; failure is never a skip.
