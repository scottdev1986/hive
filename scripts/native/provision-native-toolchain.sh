...
# Provisions the shared native build prerequisites that are NOT the compiler: the Metal toolchain check and the hash-verified Ghostty dependency cache (which lets the GhosttyKit artifact build run fully offline). The Zig compiler itself is the system `zig` on PATH, pinned by the lock and enforced by scripts/native/preflight-native-toolchain.sh and by native/sessiond/build.zig's version gate.
...
bun "$ROOT/scripts/native/ghostty-dependency-cache.ts" fetch "$ZIG" \
