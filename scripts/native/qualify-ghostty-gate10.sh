...
# Patch-series digest controls. Green: the real series digests to the locked value. Missing-patch positive control: deleting one series entry from a copied fake repo root must fail with NO digest on stdout. Do not digest a partial payload when a series entry is missing (a pipeline subshell can swallow the failure and still exit 0).
expected_series_sha=$(lock_value ghostty.patchSeriesSha256)
green_series_sha=$("$ROOT/scripts/native/vendor-ghostty.sh" patch-series-sha256)
...
mkdir -p "$TMP/series-control/scripts/native" "$TMP/series-control/native"
/bin/cp "$ROOT/scripts/native/vendor-ghostty.sh" "$TMP/series-control/scripts/native/"
...
control_status=0
control_output=$("$TMP/series-control/scripts/native/vendor-ghostty.sh" patch-series-sha256 2>"$TMP/series-control.stderr") \
  || control_status=$?
{
