...
for arch in arm64 x86_64; do
  ...
  (
    ...
    # EXECUTED + NOT-SKIPPED enforcement. xctest exits 0 even when a test is XCTSkip'd or when zero tests match the -XCTest selector, so a bare exit code is a false-green (the exact failure mode this lock guards). Require the test case to PASS-execute, forbid any "skipped", and require exactly one executed test with no failures.
    if ! /usr/bin/grep -q \
