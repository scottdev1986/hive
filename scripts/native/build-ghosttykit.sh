...
if [[ $PRODUCTION -eq 1 ]]; then
  "$ROOT/scripts/native/preflight-native-toolchain.sh" --production
else
  "$ROOT/scripts/native/preflight-native-toolchain.sh"
fi
"$ROOT/scripts/native/vendor-ghostty.sh" verify
...
PATCH_SHA=$("$ROOT/scripts/native/vendor-ghostty.sh" patch-series-sha256)
...
# Shared artifact cache across worktrees: build in unique scratch, publish atomically at the end. Never mutate $FINAL_OUT in place — a sibling worktree may be reading it right now.
...
export PATH="$ROOT/scripts/native/zig-runner-tools:$PATH"
...
# Ghostty's own xcframework build names the macOS static library without a "lib" prefix (the iOS slices already use "libghostty-internal-fat.a"). swift-package-manager's binaryTarget requires the "lib" prefix to link the archive into a final product (executable/xctest bundle) — SwiftPM accepts the unprefixed name for building a single library target, then fails only at the final test-bundle link with "unexpected binary name ... Static libraries should be prefixed with lib". Rename in place and repoint the xcframework's own manifest so every consumer sees a valid archive name. The slice directory name and archive filename are read from the plist's own macOS entry rather than hardcoded, so this survives Ghostty renaming its LibraryIdentifier (e.g. arch-list changes) or BinaryPath convention.
...
# create-xcframework receives slices from independent build jobs, so its AvailableLibraries array reflects completion order. Normalize that order before the artifact is shipped; LibraryIdentifier is the stable key.
...
# ReleaseFast still carries DWARF in static archives. Those sections embed the ephemeral worktree/cache locations from the build host, so identical inputs produce different shipped bytes and leak local paths. The release artifact needs no debugger symbols; remove only those sections after every slice has been assembled. Keep this before the checkpoint and ABI qualifications so they exercise exactly the bytes we ship.
...
if [[ "$(lock_value ghostty.symbolListSha256)" != "REQUIRED_BEFORE_TG1_PRODUCTION" ]]; then
  "$ROOT/scripts/native/check-ghostty-abi.sh" "$mac_library"
fi
...
# Gate 6 (M1-B1): qualify the headless checkpoint-authoring contract on the freshly built lib-vt slice and author the surface-restore fixture, BEFORE the manifest is written so the fixture is part of the recorded artifact.
"$ROOT/scripts/native/qualify-ghostty-checkpoint.sh" "$OUT"

bun "$ROOT/scripts/native/write-ghostty-artifact-metadata.ts" "$OUT"

if [[ $PRODUCTION -eq 1 ]]; then
  "$ROOT/scripts/native/qualify-ghostty-release-lock.sh" "$OUT"
fi

# Atomic publish; incumbent-vs-fresh lives in publish-ghostty-artifact.sh.
"$ROOT/scripts/native/publish-ghostty-artifact.sh" "$OUT" "$FINAL_OUT" "$LOCK"
