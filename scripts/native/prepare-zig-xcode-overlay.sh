...
# Symlink farm over the locked Zig's bundled Darwin stub and the active Xcode SDKs. Keyed by (zig version, locked Xcode build) and created ONCE, atomically. Do not delete-and-rebuild on every call: that yanks the overlay out from under concurrent builds in sibling worktrees. The lock pins the Xcode build (preflight asserts it), so a locked toolchain change renames the key and invalidates the overlay.
...
touch "$TMP/.complete"
# Atomic publish: rename into place; if a concurrent builder won the race the rename lands INSIDE its overlay (mv-into-directory semantics) — detect that, remove the misplaced copy, and use the winner's complete overlay.
