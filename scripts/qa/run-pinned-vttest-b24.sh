...
# vttest is normally started by an interactive shell and expects its menu input to inherit canonical CR-to-NL handling. The session-host provider profile is deliberately raw, so establish the normal shell handoff here.
stty sane
