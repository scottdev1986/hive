...
/// Semantic pane status. Unknown is explicit so an unrecognized feed word can never be silently upgraded to a healthy state. The focus ring is a separate signal and never overwrites the status border.
public enum PaneStatus: Equatable, Codable {
    ...
    /// Whether the amber pulse burst applies (bounded burst handled by the view; semantic state never depends on animation).
...
/// Bounded amber pulse: a short burst, then steady. Exposed as data so the view layer and tests agree on the bound.
