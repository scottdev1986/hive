...
The same complete pre-teardown journal is replayed byte-for-byte into a visible production `HiveTerminalView` in a titled NSWindow. The opt-in proof validates the journal hash and sequence range, observes Claude's retained U+23F8 status glyph in the semantic grid, waits for a real IOSurface presentation, then captures that window through the WindowServer. [`c1-c11-real-window.png`](c1-c11-real-window.png) is 2,536×1,600 pixels with SHA-256 `16994ebc1bf2e1281ed9e8d87ea22ce42e2ea8cf484cfbe84f8f1a404bbbd519`. Its title identifies the C1.1 proof window, and its pixels visibly contain Claude's status row rather than a fixture label or background-only pane. This is C1.1 renderer evidence; it does not relabel B2.5's separately rejected blank `spawning` capture as green.
