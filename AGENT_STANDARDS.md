...
2. Check a tool's default output cap before comparing counts. A cap is a constant, so two runs report it identically no matter what changed; a truncated diagnostic list is equally consistent with "nothing changed" and "you added six hundred". A suspiciously round number on both sides is the tell. Ask whether the number COULD have moved, not whether it did.
