...
} from "../../src/schemas/routing-derivation";
...
const effectiveDefault = (provider: "claude" | "codex") => ({
  ...
  model: unknown<string>(
    "field-absent",
    provider === "claude" ? "claude.initialize" : "codex.config/read",
    AT,
  ),
...
describe("identifyModelVendor", () => {
  test("identifies canonical ids, launch tokens, and aliases from vendor catalogs", () => {
    const discovery = {
      claude: {
        ...
        effectiveDefault: effectiveDefault("claude"),
      },
      codex: {
        status: "ok",
    ...
    expect(identifyModelVendor("gpt-5.4", discovery)).toEqual({
      state: "claimed",
      provider: "codex",
    });
  });
  ...
  test("fails closed when catalogs are unreadable, collide, or claim nothing", () => {
    const unreadable = {
      claude: { status: "unavailable", reason: "offline" },
    } as const;
    ...
    const collision = {
      ...
      grok: {
        ...
        effectiveDefault: {
          provider: "grok",
          model: unknown<string>("field-absent", "grok.models", AT),
          effort: unknown<string>("field-absent", "grok.models", AT),
        },
      },
      kimi: {
        status: "ok",
        ...
        effectiveDefault: {
          provider: "kimi",
          model: unknown<string>("field-absent", "kimi.provider/list", AT),
          effort: unknown<string>("field-absent", "kimi.provider/list", AT),
        },
      },
      opencode: {
        status: "ok",
        ...
        effectiveDefault: {
          provider: "opencode",
          model: unknown<string>("field-absent", "opencode.models", AT),
