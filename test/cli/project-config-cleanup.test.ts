...
import { repairLeakedProjectConfig } from "../../src/cli/project-config-cleanup";
    ...
    try {
      ...
      await writeFile(
        ...
        JSON.stringify({
                ...
                hooks: [
                  {
                    type: "command",
                    command:
                      "hive event turn-end --agent orchestrator --port 4483",
                  },
                ],
              },
            ],
          ...
          statusLine: {
            type: "command",
            command: "hive statusline --agent orchestrator --port 4483",
          },
        }),
      );
      await writeFile(
      ...
      await writeFile(
        join(root, ".codex", "hive-notify.sh"),
