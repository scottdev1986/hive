...
} from "../../src/cli/orchestrator-runtime";
...
describe("orchestrator runtime marker", () => {
  ...
  test("rejects a well-formed marker whose owner process is gone", async () => {
    ...
    await withOrchestratorRuntime(
      "codex",
      async () => {
        expect(await readLiveOrchestratorRuntime(path, () => false)).toEqual(
          null,
        );
      },
      { path, pid: 4321 },
  ...
  test("publishes and clears the native root session without replacing ownership", async () => {
    ...
    await withOrchestratorRuntime(
      ...
      async () => {
        expect(
          await publishOrchestratorSessionId("thread-root", path, 2468),
        ).toEqual(true);
        expect(
          (await readLiveOrchestratorRuntime(path, () => true))?.sessionId,
        ).toEqual("thread-root");
        expect(await publishOrchestratorSessionId(null, path, 2468)).toEqual(
          true,
        );
        expect(
          (await readLiveOrchestratorRuntime(path, () => true))?.sessionId,
        ).toBeUndefined();
        expect(await publishOrchestratorSessionId("wrong", path, 9999)).toEqual(
          false,
        );
      },
      { path, pid: 2468 },
