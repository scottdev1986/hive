...
describe("manifest parsing", () => {
  test("round-trips through JSON", () => {
    expect(parseReleaseManifest(JSON.parse(JSON.stringify(manifest)))).toEqual(
      manifest,
    );
  });
  ...
  test("rejects path-bearing versions and tags that disagree with them", () => {
    ...
    expect(() =>
      parseReleaseManifest({ ...manifest, tag: "v9.9.9" }),
    ).toThrow();
  });
  ...
  test("repeated universal asset names must describe the same bytes", () => {
    ...
    expect(() =>
      parseReleaseManifest({
        ...manifest,
        artifacts: [...manifest.artifacts, { ...x64, sha256: "d".repeat(64) }],
      }),
    ).toThrow();
  });
  ...
  test("rejects unknown fields within the current schema version", () => {
    expect(() =>
      parseReleaseManifest({ ...manifest, securtyCritical: true }),
    ).toThrow();
    expect(() =>
      parseReleaseManifest({
        ...manifest,
        artifacts: [
          { ...manifest.artifacts[0], downloadUrl: "file:///tmp/owned" },
        ],
      }),
    ).toThrow();
  });
...
describe("artifact selection", () => {
  test("picks the CLI for this architecture", () => {
    expect(selectArtifact(manifest, "cli", "arm64")?.name).toEqual(
      "hive-darwin-arm64",
    );
    expect(selectArtifact(manifest, "cli", "x64")?.name).toEqual(
      "hive-darwin-x64",
    );
  });
  ...
  test("matches bytes only when both size and digest agree", () => {
    ...
    expect(artifactMatches(artifact, new TextEncoder().encode("hivf"))).toEqual(
      false,
    );
  });
});
...
describe("manifest signature", () => {
  ...
  test("accepts a manifest signed by the embedded key", () => {
    ...
    expect(verifyManifest(bytes, key.signBytes(bytes), key.publicKey)).toEqual({
      verified: true,
      signed: true,
    });
  });
  ...
  test("rejects a manifest whose bytes changed after signing", () => {
    ...
    expect(verifyManifest(tampered, signature, key.publicKey)).toMatchObject({
      verified: false,
    });
  });
  ...
  test("fails closed when the signature is stripped from a signed build", () => {
    const key = releaseKey();
    expect(verifyManifest(bytes, null, key.publicKey)).toMatchObject({
      verified: false,
    });
  });
  ...
  test("a garbage public key is a refusal, not a crash", () => {
    expect(verifyManifest(bytes, "AAAA", "not-a-key")).toMatchObject({
      verified: false,
    });
  });
});
...
describe("key rotation", () => {
  ...
  test("accepts a signature from either key while both are trusted", () => {
    ...
    expect(verifyManifest(bytes, old.signBytes(bytes), both)).toMatchObject({
      verified: true,
      signed: true,
    });
    // Rotation complete: the same binary accepts the new key with no update.
    expect(verifyManifest(bytes, next.signBytes(bytes), both)).toMatchObject({
      verified: true,
      signed: true,
    });
  });
  ...
  test("a key outside the list is still refused — the list widens trust, it does not remove it", () => {
    ...
    expect(
      verifyManifest(bytes, attacker.signBytes(bytes), trusted.publicKey),
    ).toMatchObject({ verified: false });
    expect(
      verifyManifest(
        bytes,
        attacker.signBytes(bytes),
        `${trusted.publicKey},${releaseKey().publicKey}`,
      ),
    ).toMatchObject({ verified: false });
  });
  ...
  test("once the old key is dropped, its signature stops verifying", () => {
    ...
    const next = releaseKey();
    expect(
      verifyManifest(bytes, old.signBytes(bytes), next.publicKey),
    ).toMatchObject({ verified: false });
  });
  ...
  test("a valid key alongside a malformed one still verifies", () => {
    ...
    expect(
      verifyManifest(
        bytes,
        good.signBytes(bytes),
        `not-a-key,${good.publicKey}`,
      ),
