import { afterAll, describe, expect, test } from "bun:test";
import { execFileSync } from "node:child_process";
import { mkdirSync, mkdtempSync, realpathSync, rmSync } from "node:fs";
import { join } from "node:path";
import {
  projectRootOrCwd,
  resolveProjectRoot,
} from "../../src/daemon/project-identity-core/project-root";
import { OUTSIDE_REPO_TMPDIR } from "../outside-repo-tmpdir";

const dirs: string[] = [];
afterAll(() => {
  for (const dir of dirs) rmSync(dir, { recursive: true, force: true });
});

function tempDir(prefix: string): string {
  const dir = mkdtempSync(join(OUTSIDE_REPO_TMPDIR, prefix));
  dirs.push(dir);
  return dir;
}

describe("resolveProjectRoot", () => {
  test("a repo subdirectory resolves to the worktree root", () => {
    const repo = tempDir("hive-project-root-");
    execFileSync("git", ["init"], { cwd: repo, stdio: "ignore" });
    const subdir = join(repo, "nested", "deeper");
    mkdirSync(subdir, { recursive: true });
    // Compare against the physical path: git reports physical paths.
    const root = realpathSync(repo);
    expect(resolveProjectRoot(subdir)).toEqual(root);
    expect(projectRootOrCwd(subdir)).toEqual(root);
  });

  test("outside a repo the root is null and the fallback is the directory itself", () => {
    const dir = tempDir("hive-not-a-repo-");
    expect(resolveProjectRoot(dir)).toEqual(null);
    // The fallback is the literal directory passed in, symlinks and all.
    expect(projectRootOrCwd(dir)).toEqual(dir);
  });

  // The daemon calls this with no argument (HIVE_PROJECT_ROOT unset, or no
  // explicit repoRoot), so the no-argument form has to resolve the repository
  // root rather than hand back whichever subdirectory it was started in — a
  // daemon rooted one level down keys a second project identity.
  test("the no-argument form resolves the root, not the working directory", () => {
    const repo = tempDir("hive-daemon-cwd-");
    execFileSync("git", ["init"], { cwd: repo, stdio: "ignore" });
    const subdir = join(repo, "packages", "worker");
    mkdirSync(subdir, { recursive: true });
    const root = realpathSync(repo);
    const previous = process.cwd();

    try {
      process.chdir(subdir);
      expect(projectRootOrCwd()).toEqual(root);
    } finally {
      process.chdir(previous);
    }
  });
});
