...
function input(overrides: Partial<MemoryWriteInput> = {}): MemoryWriteInput {
  return {
    ...
    // Articles are born unverified: a write cannot certify itself, and these
    // cases are about raw evidence, supersession and indexing rather than
    // verification.
    status: "unverified",
    kind: "article",
    supersedes: [],
...
describe("raw observations and compiled articles", () => {
  test("one write creates immutable raw evidence, a compiled article, index, and log", async () => {
    ...
    expect(
      await readFile(join(getRepoMemoryRoot(root), "wiki", "index.md"), "utf8"),
    ).toContain("[repo/testing] the-login-test-is-flaky");
    expect(
      await readFile(join(getRepoMemoryRoot(root), "wiki", "log.md"), "utf8"),
  ...
  test("updating an article preserves both raw observations and supersession", async () => {
    ...
    const second = await writeMemoryFact(
      root,
      input({
        id: "login-flake",
  ...
  test("superseding a duplicate id removes its article and carries its raw evidence", async () => {
    const root = await makeRoot();
    const duplicate = await writeMemoryFact(
      root,
      input({
        id: "duplicate",
    ...
    const canonical = await writeMemoryFact(
      root,
      input({
        id: "canonical",
  ...
  test("rejects malformed writes before creating memory files", async () => {
    ...
    ).rejects.toThrow("topic must be lowercase kebab-case");
    await expect(
      writeMemoryFact(root, {
        ...input(),
        status: "verified",
  ...
  test("a normalized-title duplicate under a different id is rejected with an update pointer", async () => {
    ...
    expect(
      (await readMemoryFact(root, "repo", "quota-token-spend"))?.body,
  ...
  test("genuinely different titles do not collide", async () => {
    ...
    const other = await writeMemoryFact(
      root,
      input({
  ...
  test("validates ids before resolving filesystem paths", async () => {
    const root = await makeRoot();
    await expect(readMemoryFact(root, "repo", "../escape")).rejects.toThrow(
      "Invalid memory id",
    );
  });
});
...
describe("spawn index", () => {
  ...
  test("normalizes NUL bytes found in a pre-existing compiled article", async () => {
    ...
    const contaminated = (await readFile(written.path, "utf8")).replace(
      "The login test",
      "The login\0 test",
    );
  ...
  test("pitfalls beyond the cap still leave the article floor", async () => {
    const root = await makeRoot();
    for (let index = 0; index < 35; index += 1) {
      await writeMemoryFact(
        root,
    ...
    expect(rows).toHaveLength(30);
    expect(rows.filter((line) => !line.includes("[pitfall]"))).toHaveLength(8);
    expect(built).toContain("17 older articles omitted");
  });
  ...
  test("an older article matching the brief outranks newer unrelated ones", async () => {
    ...
    expect(built.indexOf("quota-reconciliation")).toBeLessThan(
      built.indexOf("login-flake"),
    );
    expect(built.indexOf("quota-reconciliation")).toBeLessThan(
      built.indexOf("deploy-freeze"),
    );
  });
  ...
  test("without a brief it degrades to pitfalls then newest, capped, no duplicates", async () => {
    ...
    expect(rows.slice(2).every((line) => !line.includes("[pitfall]"))).toBe(
      true,
    );
  ...
  test("legacy harvest-tag articles are not silently dropped from the injected index", async () => {
    const root = await makeRoot();
    // Term-dense stubs so brief relevance would promote them if they stayed
    // in the ranking pool — the failure mode R-15 measured on the real corpus.
    for (let index = 0; index < 5; index += 1) {
      await writeMemoryFact(
        ...
        input({
          ...
          topic: "pitfalls",
          kind: "pitfall",
          title: `Pitfall: quota broker reconciliation ${index}`,
          body: "Auto-harvested unreviewed claim about quota broker reconciliation.",
    ...
    await writeMemoryFact(
      ...
      input({
        ...
        kind: "pitfall",
        title: "Quota tables double-count on retry",
        body: "A reviewed pitfall about quota reconciliation.",
        tags: ["pitfall", "quota"],
        source: "agent",
      }),
    );
    await writeMemoryFact(
      ...
      input({
        ...
        body: "A reviewed article about broker reattach.",
        tags: ["daemon"],
        source: "user",
      }),
    );
    ...
    expect(catalog).toContain("stub-0");
    expect(catalog).toContain("reviewed-pitfall");

    const built = await buildMemoryIndex(root, {
      brief: "Repair the quota broker reconciliation",
    });
    const rows = built.split("\n").filter((line) => line.startsWith("- ["));
    expect(rows).toHaveLength(7);
    expect(rows.filter((line) => line.includes("stub-"))).toHaveLength(5);
    expect(built).toContain("reviewed-pitfall");
    expect(built).toContain("reviewed-article");
    expect(built).not.toMatch(/\d+ older articles? omitted/);
  });

  test("delivered rows plus the omission footer equals the stored corpus", async () => {
    const root = await makeRoot();
    for (let index = 0; index < 40; index += 1) {
      await writeMemoryFact(
        ...
        input({
          ...
          topic: "pitfalls",
          kind: "pitfall",
          title: `Pitfall: dense harvest token stew ${index}`,
          body: "Auto-harvested unreviewed claim.",
          tags: ["pitfall", "harvest"],
          source: "orchestrator",
          status: "unverified",
          verified: undefined,
    ...
    await writeMemoryFact(
      ...
      input({
        ...
        topic: "testing",
        kind: "pitfall",
        title: "Reviewed pitfall about dense harvest token stew",
        body: "Hand-written.",
        tags: ["pitfall"],
        source: "agent",
      }),
    ...
    await writeMemoryFact(
      ...
      input({
        id: "only-reviewed-b",
        topic: "daemon",
        title: "Reviewed article about dense harvest token stew",
        body: "Hand-written.",
        tags: ["daemon"],
        source: "user",
      }),
    ...
    const built = await buildMemoryIndex(root, {
      brief: "dense harvest token stew",
    });
    const rows = built.split("\n").filter((line) => line.startsWith("- ["));

    expect(rows).toHaveLength(30);
    const omitted = Number(
      built.match(/\((\d+) older articles? omitted/)?.[1] ?? "0",
    );
    expect(rows.length + omitted).toBe(42);
  });

  test("an unverified non-harvest article still reaches the injected index", async () => {
    const root = await makeRoot();
    await writeMemoryFact(
      root,
      input({
        ...
        topic: "daemon",
        title: "The broker reattach handshake as far as understood",
        body: "Hand-written unverified reconstruction — not a harvest stub.",
        tags: ["daemon"],
        source: "agent",
        status: "unverified",
        verified: undefined,
    ...
    const built = await buildMemoryIndex(root, {
      brief: "broker reattach handshake",
    });
    expect(built).toContain("rescued-mechanism");
  });
});
...
describe("legacy flat-memory migration", () => {
  test("preserves source bytes in raw and compiles current truth", async () => {
    ...
    await Bun.write(
      join(directory, "corrected-router.md"),
    ...
    expect(await readdir(join(directory, "raw", "routing"))).toEqual(
      beforeSecond.raw,
    );
    expect(await readdir(join(directory, "wiki", "routing"))).toEqual(
      beforeSecond.articles,
    );
    expect(await readFile(join(directory, "wiki", "log.md"), "utf8")).toBe(
      beforeSecond.log,
    );
    expect(await readdir(join(root, ".hive", "memory-backups"))).toEqual(
      beforeSecond.backups,
    );
  });
  ...
  test("backs up the whole corpus before a failed first compile write", async () => {
    ...
    expect(
      (await readdir(join(root, ".hive"))).filter(
        (name) =>
          name.startsWith("memory-restore-") ||
          name.startsWith("memory-failed-"),
      ),
    ).toEqual([]);
  });
});
