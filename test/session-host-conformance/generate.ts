...
export const GENERATED_FILES = {
  ...
  /// Thin HVTCP001 layout projection for HiveTerminalKit (production Wire module).
  checkpointSwift: resolve(
    import.meta.dir,
    "../../workspace/Sources/HiveTerminalKit/Wire/CheckpointHeader.generated.swift",
  ),
...
const swiftIntConstants = (
  ...
  Object.entries(value)
    .map(([name, number]) => `${indent}${access}static let ${name} = ${number}`)
    .join("\n");
...
${indent}    ${a}static let headerBytes = ${CHECKPOINT_HEADER.bytes}
${indent}    ${a}static let flags: UInt32 = ${CHECKPOINT_HEADER.flags}
...
function renderCheckpointSwift(): string {
...
${renderSwiftCheckpointNested("    ", "public")}
}
`;
}
...
${zigUsizeConstants(CHECKPOINT_HEADER.offsets, "        ")}
    };
    pub const width = struct {
${zigUsizeConstants(CHECKPOINT_HEADER.widths, "        ")}
    };
};
...
export function renderGeneratedArtifacts(): Readonly<Record<string, string>> {
  return {
    ...
    [GENERATED_FILES.checkpointSwift]: renderCheckpointSwift(),
