...
`hive init` resolves a canonical Git worktree or exact plain directory to an immutable `HiveUUID` after ensuring the repo profile is current. User linked worktrees are separate projects; Hive-managed agent worktrees attach to their authenticated owner. Nested repositories win. Plain bookmarks follow moves, so path disagreement requires rebind; recreated paths never inherit automatically. Concurrent initializations coalesce under a registry uniqueness lease.
