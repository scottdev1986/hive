...
**RESOLVED (2026-07-12).** This document originally opened with a potentially fatal risk: both measured headless runs that invoked MCP tools—one on each reachable model—executed the tool successfully and then ended with `stop_reason: "cancelled"`, and the adapter was declared unshippable until an MCP-using turn reached `end_turn` on Hive's own launch path. That gate has since been passed, and the diagnosis was not what the symptom suggested. It is recorded here rather than deleted, because the wrong conclusion is the tempting one.

The cancellations were never an MCP defect and not headless-mode as such. They were the **permission system**: in headless `-p` under the default permission mode, a tool call that would need an interactive approval is resolved `cancelled` in 0–3ms (`cancellation_category: "permission_cancelled"`, `wait_ms: 0`) — nobody was ever going to be asked — the whole turn dies, no `signals.json` is written, and the process still exits 0. Which tools need approval is profile-dependent, which is why one model appeared to work and the other did not.

`--always-approve` (the analogue of the `bypassPermissions` mode Hive already uses for Claude writers) eliminates it on every path, including the tmux/TTY path Hive actually launches agents on: measured `turn_completed` with `stop_reason: "end_turn"`, `signals.json` written, MCP answers matching the live graph. Read-only agents do not need it: a `--deny` rule produces a *clean refusal* the model absorbs, and the turn still completes `end_turn` — deny and cancel are different outcomes. Both Hive roles therefore exist on the TUI path, and ACP is not required for safety.

The measurements are in `docs/research/grok-mcp-turn-completion.md` (commit 9289210), against `grok 0.2.93 (f00f96316d4b)`. §12's end-to-end gate below still stands as an acceptance test — it must pass on Hive's real argv, with the permission flags — but it is a test to run, no longer an open question about whether the vendor can work at all.
...
**That directory is `.agents/skills` — the one Codex already reads.** Measured 2026-07-12 (codex-cli 0.144.1, grok 0.2.93 `f00f96316d4b`) with uniquely-named probe skills, asking each CLI what its *model* sees rather than what a lister lists: `codex debug prompt-input` places a foreign skill's name and description directly in the model-visible prompt, and a Grok turn with every tool denied still produced a probe token that existed only in its skill catalog. Grok additionally reads `~/.grok/skills`, `~/.grok/bundled/skills`, and the user-level `~/.agents/skills`; it does not read `~/.codex/skills`, and Codex does not read `~/.grok/skills`.

So `shippedSkillsFor` decides what Hive WRITES and nothing about what a CLI READS. Two consequences, and they have different answers:

- **Agents (fixed).** `provisionSkills` removes the contracts Hive ships for other vendors from the vendor's directory in that worktree, touching only Hive's own unmodified bytes. Skill directories are per-checkout state (`.agents/`, `.claude/` are gitignored), so a fresh worktree is clean and only a reused one could carry a predecessor's contract. A Grok agent therefore never reads Codex's contract, or vice versa. This is a mechanism, not a label, and it is what the release depends on.
- **The primary checkout (fixed by withholding).** `hive init` no longer installs a vendor CONTRACT into a native directory that a second *installed* vendor also reads. The rule needs no "is this a contract?" flag — it falls out of the manifest: a skill may be written into a directory only if it is addressed to every vendor that reads it (`skillAddressesEveryReader`). The vendor-neutral skills (`hive-memory`, `karpathy-guidelines`, shipped to every vendor) therefore still install everywhere, while `hive-codex` and `hive-grok` are withheld from `.agents/skills` when both CLIs are present — and `init` says so, because a skill that quietly did not install reads exactly like one that failed to. The condition is **conditional on a second vendor sharing the directory**: with one CLI installed, or in an agent's worktree, nothing changes at all. Justification: the contract's first line addresses an agent spawned into `.hive/worktrees/`, so a human's hand-run session in the repo root was never its reader, and agents still receive it per-worktree.

  Moving a contract into a vendor's private user scope was measured and rejected twice over: for one vendor it fixes only one direction of a symmetric leak (Grok would still read Codex's contract from the shared project dir), and for both vendors it mutates user-global vendor state — a non-goal under §15 — loading a Hive contract into every session the user runs in every repo, including repos with no Hive in them. See `src/adapters/skills.ts` for the full record.

- **What remains true anyway (accepted residual).** The *user's own* skills, and any non-Hive skill either CLI finds in `.agents/skills`, are still visible to both. Hive withholds only what Hive ships; it does not police a shared directory it does not own, and it will not hide or move a human's files to simulate isolation. Two vendors reading one directory is a property of the vendors, and the guarantee Hive makes is narrower and honest: **no agent Hive spawns will find another vendor's Hive contract in the directory its CLI reads.**

A skill's `description` naming its vendor in the first clause is defence in depth against the residual, and is explicitly not the fix.
...
The first end-to-end gate uses the exact interactive launch argv Hive will ship — **including its permission flags** — invokes at least one Hive MCP tool and one Graphify MCP tool, observes successful tool results, and then requires the correlated turn boundary to carry `stop_reason == "end_turn"` on both reachable models. Successful tool output alone is insufficient: a cancelled turn executes its tools and *then* dies, which is exactly how the original failure looked. Failure of this test blocks the entire adapter because an agent that cannot finish an MCP-using turn cannot reliably send, land, or use the graph.

This gate has been passed on the measured path (`--always-approve` for writers, `--deny` rules for readers; see §1), so it is now a regression test rather than an open risk. It keeps its blocking status: the permission flags are what make it pass, and an argv that loses them fails silently — with a zero exit code.
...
| An MCP tool succeeds but the Grok turn cancels afterward. | On Hive's exact interactive launch path *with its permission flags*, invoke Hive and Graphify MCP tools on both reachable models; require successful results followed by correlated `turn_completed` with `stop_reason: end_turn`. Any `cancelled` result blocks the adapter. Root cause is known (§1): an argv that drops `--always-approve`, or a read-only argv whose `--deny`/`--allow` rules leave an approval-needing tool uncovered, reintroduces it — and the process still exits 0. |
| A tool nobody can approve is read as a user's decision, or as flaky. | Feed a `permission_cancelled` fixture (`decision: "cancelled"`, `wait_ms: 0`, no `signals.json`, exit 0); assert Hive reports a launch-configuration failure naming the tool, and never a completed turn, a user cancellation, or a retryable error. |
...
| A Grok agent silently reads CODEX's Hive contract (or vice versa) because both CLIs scan `.agents/skills`. | Provision a worktree for one vendor after the other has used it, then assert the other vendor's contract is ABSENT from the directory — not merely deprioritised. `shippedSkillsFor` filters what Hive writes and nothing about what the CLI reads, so a per-vendor install is not evidence. Fixed by the prune in `provisionSkills`. |
| `hive init` puts both vendors' contracts in the shared `.agents/skills`, and each CLI reads the other's. | Run init with Codex and Grok both detected; assert `hive-grok` is absent from `.agents/skills` and REPORTED as withheld, while `hive-memory` (shipped to every vendor) is present. Fixed by `skillAddressesEveryReader`. **The end-to-end control belongs to this commit and could not be written before it:** with the vendor added, run `codex debug prompt-input` in an inited repo and assert the model-visible prompt contains no `hive-grok` — that instrument answers "does the MODEL see it", which a directory listing does not. Mutation: put the contract back in the shared directory and the test must fail. |
| The withholding rule is "simplified" into never installing a single-vendor skill. | Assert `skillReaders("codex", ["claude"])` is `["codex"]` — Claude and Codex do NOT share a directory, so a single-vendor user and every user who exists today must see no change. The rule bites only where two vendors genuinely read one directory. |
...
- ~~**Top risk:** whether Hive's exact interactive Grok launch can finish an MCP-using turn with `stop_reason == "end_turn"`.~~ **RESOLVED 2026-07-12** (§1): it can, on every path, once permissions are pre-authorized. The cancellations were `permission_cancelled`, not an MCP defect. Writers use `--always-approve`; readers use `--deny` rules, whose refusals are clean and still complete the turn. Measured in `docs/research/grok-mcp-turn-completion.md` (9289210);
- whether `--allow "MCPTool"` pre-authorizes **grok-4.5's** `use_tool` MCP wrapper is `UNKNOWN` — the allow-rule measurement ran on the composer profile, whose `CallMcpTool` is auto-allowed anyway, and grok-4.5's wrapper is the one that *did* require approval in the original failing runs. One prompt settles it, and it gates read-only reviewers on grok-4.5;
- `--permission-mode dontAsk` semantics for an approval-needing tool covered by no rule is `UNKNOWN`; no reachable tool exercised the ask-path. Do not rely on it as a deny-backstop;
