...
- **The primary checkout (accepted residual).** After `hive init` with both CLIs present, both contracts sit in the shared `.agents/skills` and each CLI sees the other's. Hive cannot hide one without uninstalling it for the vendor that wants it, so this is a property of the vendors rather than a Hive defect, and its readers are the human's own sessions and the orchestrator — whose instructions ride its spawn prompt, not a skill file. Moving a contract into a vendor's private user scope was measured and rejected twice over: for one vendor it fixes only one direction of a symmetric leak (Grok would still read Codex's contract from the shared project dir), and for both vendors it mutates user-global vendor state — a non-goal under §15 — loading a Hive contract into every session the user runs in every repo, including repos with no Hive in them. See `src/adapters/skills.ts` for the full record.
