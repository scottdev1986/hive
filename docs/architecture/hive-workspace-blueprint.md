...
Five hypotheses were named. Three have run — the native transcript pane, authenticated IPC, and identity under motion — and their results are folded into the sections they belong to rather than parked here. Two can still overturn important parts of the plan:
...
2. **Provider conformance.** Run the same lifecycle, approve, deny, needs-user, steer, cancel, resume, invalid-model, and read-only fixture against every binding generation, recording whether each fact is documented, observed, and billable.

Of the three that ran, only identity under motion overturned anything: it confirmed the six-step resolver order and the tombstone rule, and rejected the bookmark mechanism those rules were going to be built on.
