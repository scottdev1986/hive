...
Five hypotheses were named; four can still overturn important parts of the plan, and one is now settled:
...
2. **Native transcript pane.** The [`workspace/` prototype](../../workspace/docs/prototype-hypothesis-2-evidence.md) drove streaming partial messages, 5,000-line tool output, ANSI content, missing provider fields, approvals, and inline diffs from a mock structured source through an NSTextView transcript, and the hypothesis survived: find, selection, links, IME, and VoiceOver come from the platform rather than reimplementation; huge output collapses behind an explicit control instead of flooding scrollback; approvals stay actionable in place instead of scrolling away as a keystroke race. Still unproven, and the only things that could reopen the terminal-renderer decision: a human comprehension comparison, a screen-reader and IME conformance audit, transcript behavior at soak scale, and interactive subprocesses that expect a TTY (deferred to the SwiftTerm shell pane).
3. **Authenticated IPC — proven.** The [authenticated-XPC prototype](../../prototypes/authenticated-xpc/) showed with runnable evidence that a public XPC signing requirement rejects a signed-but-untrusted client holding a real endpoint, that an anonymous endpoint alone does not authenticate, that connection-bound capabilities restrict subject/action with no confused deputy, and that `CLOEXEC` descriptors do not reach a hostile grandchild. The design in "Authenticated IPC and capabilities" stands; this hypothesis no longer threatens the plan.
...
- [Authenticated-XPC prototype](../../prototypes/authenticated-xpc/) — runnable evidence for hypothesis 3
