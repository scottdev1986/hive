...
- **R3 — Add `effort` to the Claude route column** (`ClaudeRouteSchema`), plumbed the same way Codex's already is. Both halves are verified against claude 2.1.206 on this machine: the launch-time setter is `--effort <low|medium|high|xhigh|max>` (a real CLI flag, session default `high`), and per-model effort support is machine-discoverable at zero cost via the stream-json `initialize` control request (`supportsEffort`, `supportedEffortLevels` — first documented in `research/cross-vendor-architecture-review.md`). Mechanics and the verified probe are in `docs/model-selection.md`.
