...
- **A staleness fingerprint.** Cheap-to-recompute signals over the profile's own inputs — the doc inventory, the manifests and lockfiles, the conventional entry files — plus the date and hive version that produced it, so a later session can decide "still correct?" in a few `stat`s. The one trap, which Hive fell into and had to dig out of (SPEC §14): do *not* fold the Git tree hash into that signal. It is tempting because one hash covers every committed change, and it is wrong, because it makes every commit to every file invalidate a profile whose derived answers did not move. A fingerprint must hash what *determines* the output, or "stale" stops meaning "wrong".

We also expected the profile to carry a **repo-size-aware index budget** — aider's `--map-tokens` made explicit, scaled so a 20k-file monorepo's index could not drown every context. Hive built it and then deleted it: nothing ever read the number, and caching it meant every commit that added a file invalidated the profile. The lesson generalizes past this field. aider recomputes its map budget per run, and that is the right shape: a size-derived cap belongs at the point of use, not frozen into a durable artifact.

Two cross-cutting lessons shape the SPEC design. **Derive-then-refine, never block:** every tool treats generation as a starting point a human refines. And **cheap deterministic facts and expensive model-authored facts are different tiers:** build commands come from a manifest for free; a conventions narrative or a ranked symbol map costs a model pass. Hive splits the two tiers by *who runs them*. The deterministic tier is not a command at all — profiling costs 56 ms and zero model tokens, so every session boundary just does it, silently, regenerating whenever the inputs drift; there is no init step to run and no refresh to remember. `hive init` is left owning only the tier that must be asked for, because it writes into the repo or spends tokens: the `AGENTS.md` scaffold and seeded narrative facts. Where the other tools land on "generate once, let a human maintain it," the measurement pushes Hive somewhere the survey did not: a fact this cheap to re-derive should never have been a human's to maintain. That is the core of SPEC §14.
