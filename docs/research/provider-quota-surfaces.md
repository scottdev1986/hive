...
**"After" is a position in the ledger's commit sequence, not a wall-clock time.** Every spend row carries the number it was committed at, and every reading stores the last spend it could already have counted; what Hive adds is everything past that boundary. The rejected alternative is the obvious one — compare the spend's timestamp to the reading's — and it cannot work, because a wall clock is not a happens-before relation. Two events that share a millisecond have no order in it at all, and a strict `occurredAt > observedAt` therefore drops a spend that lands in the reading's own millisecond: not in the provider's snapshot, not in the spend since, gone. Widening it to `>=` only moves the same failure to the other side and double-counts. A sequence has the ordering the clock never had.

At the boundary the tie goes to counting, deliberately, because the two directions of error do not cost the same. Over-counting refuses a spawn that would have fit, and the next reading takes the refusal back minutes later. Under-counting spends quota the user does not have, admits a spawn past a real limit, and nothing downstream ever corrects it. Hive pays the cheap error — which is the rule below applied, not an exception to it.
