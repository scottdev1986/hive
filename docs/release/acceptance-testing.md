...
2. The first bare absolute-path `"$HIVE"` creates a new `$HOME/.hive/instances/run-<uuid>` home, daemon, ephemeral port, database, session namespace, instance id, and Workspace process for `hive-test-project`.
