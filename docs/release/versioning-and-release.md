...
The release builder inlines version, commit, date, and build hash unconditionally, and the public key when configured (`src/release/build.ts:130-149`). `HIVE_UPDATE_REPO` is not inlined; it remains a runtime value with the default `scottdev1986/hive` (`src/version.ts:54-55`).
...
`~/.local/share/hive/versions/<version>/` holds an immutable tree per release: the `hive` binary, `HiveWorkspace.app`, and `release-verification.json` together. The verification file contains the exact signed manifest bytes and signature needed to prove an offline rollback target. `current` is a symlink at one version directory and `~/.local/bin/hive` points at `current/hive` forever (`src/update/install.ts:54-83`, `:215-250`).
