...
The lock and ephemeral port are one design unit. An ephemeral port alone would let two daemons start against one home and overwrite each other's lifecycle files. The lock is acquired with exclusive creation before daemon state is opened; a matching live handshake or a recently started live owner protects it (`src/daemon/lifecycle/daemon-lifecycle.ts:21-25`, `:121-159`, `:182-188`).

The handshake binds instance identity together with build, wire protocol, schema epoch, and project identity. A client given the wrong port refuses it instead of silently routing to a sibling (`src/daemon/lifecycle/handshake.ts:70-82`, `:89-115`, `:131-150`). Long-lived helper commands receive both `--port` and `--instance-id`; the port is an address, while the handshake is proof of which daemon answered.
...
A live PID is not ownership proof because PIDs are reused. Unknown state preserves work and blocks destructive operations (`src/daemon/lifecycle/daemon-lifecycle.ts:48-83`).
...
| Instance | daemon lock/PID/port, terminal sessions, provider session sockets | Instance suffix plus handshake identity |
...
The default home remains compatible with pre-instance state. Policy inheritance reads the default database through SQLite's read-only WAL-aware connection and writes a normal audited `import-default-policy` revision into the named database (`src/daemon/routing-service/instance-settings.ts`, `src/daemon/routing-policy-store.ts`). It does not copy the database file, share a writer, import a provisional no-consent policy, or overwrite a named policy after its own first edit.
...
- Landing holds `hive-landing.lock` in the Git common directory across the complete fast-forward-only landing operation. Release is token-scoped, so one process cannot remove a successor's lease (`src/daemon/landing/landing-service.ts:115-152`, `:476-482`).
...
Splitting only reservations and observations would break the current transactional invariants. Reservation reconciliation, usage sequencing, integrity triggers, and observation watermarks form one atomic system. The shared database uses WAL, a five-second busy timeout, and `BEGIN IMMEDIATE` transactions so concurrent daemons serialize check-and-reserve (`src/usage-service/quota-ledger.ts: QuotaDatabase, reserveGroupUnchecked`).

Every reservation records `instanceId` and `instanceHome`. This prevents same-named agents in sibling instances from aliasing and gives reclamation an owner to check (`src/usage-service/quota-ledger-records.ts: ReservationSchema`, `src/usage-service/quota-ledger.ts: QuotaLedger.expired`). Reclamation follows lock-and-handshake liveness, never elapsed time:
...
An instance between lock acquisition and handshake publication is therefore protected (`src/usage-service/quota-ledger.ts: QuotaLedger.expired`). The default instance's legacy quota tables are copied once into the shared ledger without deleting the old rows (`src/usage-service/quota-ledger.ts: migrateDefaultQuotaLedger`).
...
The refusal names the instance and the observed agents or unknown marker (`src/daemon/lifecycle/instances.ts:115-143`, `src/cli/update.ts:224-231`).
