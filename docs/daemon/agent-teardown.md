...
`killAgentTeardown` (`src/daemon/server.ts:1752-1940`):
...
The recycled-PID guard matches process identity at the start of argv, not text anywhere in the command line: `basename(argv[0])` must be `codex` and `argv[1]` must be `app-server`. A prompt containing the words “codex app-server” can never satisfy that check (`src/adapters/tools/codex-app-server.ts:953-969`).
