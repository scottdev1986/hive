...
Shutdown now closes **every live agent through the same kill path**, before the timers stop (teardown needs delivery and quota alive), and then reaps the orchestrator's own pane tree — the orchestrator has no agents row, so nothing else would have. Measured end to end: **0.97s** from SIGTERM to daemon exit with two live agents and an orchestrator, no processes left, no tmux sessions left, lifecycle files cleaned, every agent `dead` in the database.
