...
`killAgentTeardown` (`src/daemon/server.ts`):
...
**Verified on a real tick, not just in a unit test.** A unit test over production-generated filenames proves the *name* parses; it does not prove the timer fires, finds the file, resolves the agent, verifies the binary, and kills the host. So: a genuinely orphaned process whose `ps` line really reads `codex app-server --listen unix://…` (a fake name would be skipped — the reaper checks the binary before it kills), a dead agent row, the pidfile at its production path, and then nothing but waiting. The maintenance tick is **30 seconds** (`server.ts`, `setInterval(…, 30_000)`). The host died, the socket and pidfile were removed, and the orchestrator was told. Run twice, because the first orphan died one second into the poll — which proves it *fired*, not that a *timer* drove it; the second, staged with clean phase, was reaped 8s later. Both inside one tick window.
