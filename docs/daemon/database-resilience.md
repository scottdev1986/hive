...
Each active reservation records its owning `instanceId` and `instanceHome`. Reconciliation consults that instance's daemon lock and handshake: only a positively dead owner makes a reservation reclaimable; live and unknown owners are preserved (`src/daemon/quota-ledger.ts:1594-1617`). A wall-clock expiry is not authority to cancel work that a sibling may still own.
...
- `PRAGMA busy_timeout = 5000` is set on **every** connection, including read-only ones — it is connection-local and writes nothing (`src/daemon/db.ts:370-373`). bun:sqlite's default is zero, which fails instantly on honest transient contention.
- `PRAGMA journal_mode = WAL`, `foreign_keys = ON` (`src/daemon/db.ts:394-396`).
- **A missing database file is refused, not recreated.** If the out-of-DB identity marker exists but the file does not, the constructor throws `HiveDatabaseIdentityError` rather than letting `{create: true}` manufacture an empty replacement (`src/daemon/db.ts:349-363`). A *replaced* database is caught too: the stored identity in `meta` is compared against the marker and a mismatch closes the handle and throws (`src/daemon/db.ts:374-393`). This is the file-absent row of the table above, closed.
- **`/health` actually reads the database.** It runs `quickCheck()` and reports `ok` / `degraded` / `unreadable`, and returns 503 when not ok (`src/daemon/server.ts:2315-2345`; `src/daemon/db.ts:721-724`). It used to return `{ok: true}` without touching SQLite — green on a corpse.
...
- **No extended retention.** The only pruner is `pruneHistory` (`src/daemon/db.ts:1576-1600`), called from the maintenance tick (`src/daemon/server.ts:1214-1233`): 14 days, deleting `events`, `messages` **only in state `applied`**, and `approvals` **only** when not `pending`. Mail addressed to an agent that died is never delivered, never applied, and therefore never pruned. `agents` rows are large and effectively immortal.
