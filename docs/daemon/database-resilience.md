...
- `PRAGMA busy_timeout = 5000` is set on **every** connection, including read-only ones — it is connection-local and writes nothing (`db.ts:370-373`). bun:sqlite's default is zero, which fails instantly on honest transient contention.
- `PRAGMA journal_mode = WAL`, `foreign_keys = ON` (`db.ts:394-396`).
- **A missing database file is refused, not recreated.** If the out-of-DB identity marker exists but the file does not, the constructor throws `HiveDatabaseIdentityError` rather than letting `{create: true}` manufacture an empty replacement (`db.ts:349-363`). A *replaced* database is caught too: the stored identity in `meta` is compared against the marker and a mismatch closes the handle and throws (`db.ts:374-393`). This is the file-absent row of the table above, closed.
