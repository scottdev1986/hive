...
`hive claude`, `hive codex`, and `hive grok` (`src/cli.ts:331-350`) **also open the Workspace**, with a read-only orchestrator of the named vendor. They are not a plain-terminal escape hatch — no such mode exists. Grok is a first-class vendor alongside Claude and Codex (`--orchestrator claude|codex|grok`, `workspace/Sources/HiveWorkspace/LaunchConfig.swift:84-87`). `hive init` is the headless onboarding boundary: skills/memory/Graphify enrichment plus daemon, no window. **Today** it still runs the outgoing deterministic profile pass and supports `--refresh` (`src/cli/init.ts`, `src/cli.ts`); long-term profile authorship is daemon-coordinated agent-authored work (SPEC decision 14), and plan package P5 removes the init/legacy path.
