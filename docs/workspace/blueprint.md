...
`hive claude`, `hive codex`, and `hive grok` **also create fresh instances and open new Workspaces**, with a read-only orchestrator of the named vendor. They are not a plain-terminal escape hatch — no such mode exists. Grok is a first-class vendor alongside Claude and Codex (`--orchestrator claude|codex|grok`, `workspace/Sources/HiveWorkspace/LaunchConfig.swift`). `hive init` is the headless, repo-only onboarding boundary: skills, optional memory, Graphify, and the outgoing deterministic profile pass. It starts no daemon and opens no window. **Today** it still supports `--refresh` (`src/cli/init.ts`, `src/cli.ts`); long-term profile authorship is daemon-coordinated agent-authored work (SPEC decision 14), and plan package P5 removes the init/legacy path.
