...
**Survival and presentation are separate.** An unexpected UI crash cannot stop an agent: agents live in daemon-owned tmux sessions, and relaunching the Workspace reattaches to them. Deliberate close actions are different. Closing an agent pane asks the daemon to kill that agent through `hive kill`; if verified teardown fails, the pane returns with the reason. Closing the Workspace runs `hive stop`, which stops the instance's team and daemon. This preserves the *viewers-not-containers* rule ([SPEC.md](../../SPEC.md) §9) without making an explicit close button a no-op. Typing in a pane is typing into the vendor's real composer, so "send the agent a message" is not a feature Hive implements — it is the TUI working (`ProjectWindowController.swift:125-181`, `AppDelegate.swift:309-350`).
