...
**Survival and presentation are separate.** An unexpected UI crash cannot stop an agent: agents live in daemon-owned sessions, and relaunching the Workspace reattaches to them. Deliberate close actions are different. Closing an agent pane asks the daemon to kill that agent through `hive kill`; if verified teardown fails, the pane returns with the reason. Closing the Workspace runs `hive stop`, which stops the instance's team and daemon. This preserves the *viewers-not-containers* rule without making an explicit close button a no-op. Typing in a pane is typing into the vendor's real composer, so "send the agent a message" is not a feature Hive implements — it is the TUI working (`workspace/Sources/HiveWorkspace/ProjectWindowController.swift:125-181`, `workspace/Sources/HiveWorkspace/AppDelegate.swift:309-350`).
...
Bare `hive` is the graphical front door: it resolves and prepares the repository, selects that repository's instance (`instances/repo-<projectKey>`), starts or reuses that instance's daemon, and opens a Workspace process — or focuses the existing one if this repo already has a window (`src/cli.ts` → `runWorkspace()` → `startSession()`). One Hive is active per repository. Different repositories may run at the same time. The project root is the git toplevel of the invocation directory, resolved by the same probe the identity resolver uses, so CLI and daemon can never disagree about the root. Outside a git worktree the app launches with no project and shows a new placeholder window — the project-neutral home a Dock click gets (`workspace/Sources/HiveWorkspace/LaunchConfig.swift`, `workspace/Sources/HiveWorkspace/AppDelegate.swift`). A fully standalone launcher whose cwd was deliberately irrelevant was tried and rejected: it left no path that could ever open a project window, with the app telling you to run `hive init` and `hive init` telling you to run `hive`.

`hive claude`, `hive codex`, and `hive grok` take the same session boundary, with queen as a read-only orchestrator on the named vendor. They are not a plain-terminal escape hatch — no such mode exists. Grok is a first-class vendor alongside Claude and Codex (`--orchestrator claude|codex|grok` selects the vendor; the root is still named queen; `workspace/Sources/HiveWorkspace/LaunchConfig.swift`). `hive init` is the headless, repo-only onboarding boundary: skills, narrative memory, embeddings, and the Graphify code graph. It starts no daemon and opens no window.
...
The feed is a terminal-free, long-lived `hive workspace-feed` subprocess printing NDJSON snapshots. It reads the daemon through the user credential and carries the agent records, writer-autonomy dial, and queen (orchestrator) status to the app; it neither registers a viewer nor changes daemon behavior (`src/cli/workspace-feed.ts:1-26`, `:132-200`). Agent panes exist only in the Workspace, attached to instance-owned terminal sessions.
...
The decision would be reopened by a terminal accessibility-conformance failure or by vendor TUIs becoming un-embeddable — **not** by the transcript evidence, which was never the problem. Evidence that a thing *works* is not evidence that it should *ship*.

Two corollaries: terminal scraping is never authoritative. The earlier decisions to keep the external terminal substrate and defer libghostty are superseded by [ADR 0001](../adr/0001-native-terminal-foundation.md) and [terminal-stack-transition.html](../design/Done/terminal-stack-transition.html) §§01/§05: pinned libghostty behind the bounded Hive bridge is binding. Alacritty, `libghostty-vt`, and WezTerm's crates are parser/core layers, not AppKit surfaces.
...
- **Gate 10.** UI and broker loss preserve execution — daemon-owned sessions for interactive agents, AgentHost for driven sessions; host/provider loss yields replay, native resume, or explicit `UNKNOWN_OUTCOME`, never duplicate work.
- **Gate 13.** No release bundle contains a fixture-driven surface. Component smoke checks may run earlier, but release acceptance must drive the installed *visible Workspace*—real daemon, real terminal sessions, real panes, and separate text/submit keystrokes. **A mock reaching a user as the product, or a headless check being reported as GUI acceptance, is a release-blocking defect.**
