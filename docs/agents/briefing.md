...
`ensureProfile` runs at every session boundary — bare `hive` and the vendor-specific Workspace commands through `startSession` (`src/cli/start.ts:125`), the orchestrator (`src/cli/orchestrator.ts:192`), the daemon (`src/daemon/server.ts:1077`), every spawn (`src/daemon/spawner-impl.ts:1880`), and `loadBriefConfig` itself (`src/adapters/brief.ts:26`). It is **silent**: there is no init step to run and no refresh to remember.
