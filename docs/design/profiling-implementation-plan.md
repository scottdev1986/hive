...
This plan covers profiler tools and bootstrap spawning (Slice 2), first-run UX and the spawn gate (Slice 4), and drift/refresh (Slice 5). It builds on Erica's Slice 1 schema and storage landed at `3832ea6`. The `profiling` routing category and cross-ecosystem fixtures are also on `main` (`src/schemas/routing-policy.ts:34`, `src/adapters/project-fixtures.test-support.ts`).
...
| `computeProfileInventory` | `src/daemon/project-profile.ts:182-200` | Canonical content-based inventory used for run validation; F1 hardens and shares it with session drift checks and `profile_inventory`. |
...
| `submitProfile` | `src/daemon/project-profile.ts:453-585` | Validate outside the lock, re-check run ownership, stage bytes, then re-prove digest and cited paths inside the lock immediately before the rename. |
...
| daemon validation | `src/daemon/project-profile-validate.ts:77-230`, `:397-553` | The daemon validates identity/provider/model, rejects silent empty profiles, resolves path containment, and remains the acceptance authority. |

### Foundation contract and remaining corrections before tool/coordinator work

The first paragraph records guarantees delivered in `3832ea6`; the numbered items are remaining contract fixes, not optional polish, and should land as the first independently-landable follow-ups.

The one-job compare-and-begin primitive is delivered in `3832ea6`: all `state.json` read-modify-writes use `withProfileLock` (`src/daemon/project-profile.ts:344-360`) over the hardened cross-process `withFileLock` (`src/adapters/file-lock.ts:147-197`), and `beginProfiling(root, profiler, {ifIdle:true})` performs the check, content inventory, and state write under it (`src/daemon/project-profile.ts:377-440`). The coordinator must always use that overload for normal first/refresh work. `readProfileState` first reconciles a matching accepted `current.json` left behind by a crash (`:271-335`), so `null` always means a live durable run record: map it to coalesced/pending, never a new launch. The default overload supersedes and is reserved for a run P2 has positively proved expired/dead; the foundation intentionally implements no expiry policy. Submission validates expensive inputs outside the lock (`:476-498`), re-checks ownership, stages the candidate, and calls `proveProfileStillHolds` inside the lock before the final rename (`:500-569`). That guarantee is “citations observed valid by the final pre-commit proof,” not “valid now”: the proof-to-rename instant cannot be closed because repo writers do not take `profile.lock`. P7 owns immediate post-commit drift detection. Validation also resolves command/workspace containment through real paths (`src/daemon/project-profile-validate.ts:520-551`). Keep these foundation regression tests green; do not reimplement them in the coordinator.

1. **Use one safe, bounded inventory policy.** The landed implementation hashes contents and skips a small directory set (`src/daemon/project-profile.ts:77-95`, `:138-200`), but it is unbounded and does not exclude credential stores or known secret files. Export an inventory service used by digesting and MCP reads. It must:
...
2. **Separate model-authored candidate from daemon envelope.** Add `ProjectProfileCandidateSchema` containing only authored fields. It must preserve the foundation's claim shapes: `docs.briefable` entries are `{path, evidence, confidence}` (`src/schemas/project-profile.ts:141-164`) and `conventionFiles` entries are `{path, kind, evidence, confidence}` (`:166-182`). The `profile_submit` payload must not choose `schemaVersion`, `generatedAt`, project UUID/input digest, provider/model/agent, run ID, tool session ID, request provenance, or accepted time. The handler assembles those fields from the authenticated active run, observed provider session, and daemon clock before calling `submitProfile`; provider/model must be copied from the run, never trusted from model output. Keep `submitProfile`'s provider/model identity check at `src/daemon/project-profile-validate.ts:180-198` as defense in depth. The current full-payload path at `src/daemon/project-profile.ts:471-498` otherwise makes the model claim daemon-owned metadata.
...
4. **Return stable machine-readable failures.** Preserve a `code` plus exact human `detail`. Routing currently throws a multiline string from `src/daemon/spawner-impl.ts:1823-1830`; store it verbatim as `routing-refused` rather than parsing it into invented structure.
...
|---|---|---|---|---|
| F1 | Safe bounded inventory snapshot/catalog/content service | Foundation inventory at `src/daemon/project-profile.ts:182-200`; new inventory-policy tests | landed foundation | same-size edit changes digest; tracked/untracked secrets, symlinks, binaries, and every cap covered |
| F2 | Submission/run follow-up: candidate/envelope split and requester/guidance provenance | `src/schemas/project-profile.ts:141-182`, `:267-305`, `:336-394`; `src/daemon/project-profile.ts:364-585`; `src/daemon/project-profile-validate.ts:77-230` | landed `3832ea6` | candidate preserves claim shapes; daemon authors run/provider/model metadata; guidance persists; `missing-unknowns` and spoofed provenance reject; delivered concurrency/re-proof/reconciliation tests remain green |
| P1 | Capability role/actions, wire schemas, `ProfileControl` seam, five MCP tools, internal gate/bypass routes | `src/daemon/capabilities.ts:19-132`, `src/daemon/server.ts:938-953`, `:2375-2406`, `:3267-3914`, `src/daemon/credentials.ts:29-83`, auth docs/tests | F1 + F2 | exact role matrix/tool visibility; cross-project/completed token denied; all rejection codes are lossless; unreadable current is null; stale current remains readable and gate-valid |
...
- foundation contract: keep Erica's four-way in-process/cross-process `ifIdle`, provider/model spoof, `missing-unknowns`, commit-time deletion, crash reconciliation, and symlinked-cwd tests green; F2 adds candidate-envelope tests proving the model cannot author those metadata fields;
