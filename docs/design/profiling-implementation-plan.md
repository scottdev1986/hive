...
This plan covers profiler tools and bootstrap spawning (Slice 2), first-run UX and the spawn gate (Slice 4), and drift/refresh (Slice 5). It builds on Erica's Slice 1 schema and storage landed at `3832ea6`, plus **landed F1** (bounded inventory) and **landed F2** (candidate/envelope and requester/guidance provenance). The `profiling` routing category and cross-ecosystem fixtures are also on `main` (`src/schemas/routing-policy.ts:34`, `src/adapters/project-fixtures.test-support.ts`). Open implementation work in this document starts at **P1**.
...
## Landed Slice 1 contract

The foundation landed on `main` at `3832ea6`. Integration packages should import these functions rather than recreate profile storage:

All file:line pointers below into `src/schemas/project-profile.ts`, `src/daemon/project-profile.ts`, `src/daemon/project-profile-validate.ts`, and `src/daemon/project-state.ts` are relative to landed `3832ea6`. Resolve each cited symbol by name and re-check its current source after rebasing before editing; line numbers are orientation, not an API.

| Interface | Landed location | Required semantics |
|---|---|---|
| `ProjectProfileSchema`, lifecycle/state schemas | `src/schemas/project-profile.ts:141-182`, `:228-305`, `:322-394` | Strict versioned validated profile and `unprofiled | profiling | current | stale | failed` state. `docs.briefable` and `conventionFiles` are evidenced claims, not path strings. |
...
| `projectProfileDir`, `currentProfilePath`, `profileStatePath` | `src/daemon/project-profile.ts:49-58` | Exact paths used in disclosure and operator status. |
| `computeProfileInventory` | `src/daemon/project-profile.ts` (F1) | Canonical **bounded** content-based inventory: dependency/credential denylists, secret basename skips, no symlink dereference for content, fail-closed caps (`inventory-limit`). Shared by run digests and (once P1 lands) `profile_inventory`. |
| `readCurrentProfile`, `readProfileState` | `src/daemon/project-profile.ts:254-335` | `readCurrentProfile` returns validated profile-or-null, including null for unreadable JSON; `readProfileState` logically reconciles an accepted `current.json` whose matching run was left `profiling` by a crash. |
| `beginProfiling` | `src/daemon/project-profile.ts:377-440` | Daemon mints run ID and input digest without removing current. `{ifIdle:true}` atomically returns one handle or `null` under the cross-process profile lock; the default overload supersedes. |
| `submitProfile` | `src/daemon/project-profile.ts` (F2) | Accepts a **candidate** plus daemon-owned envelope fields from the active run; validate outside the lock, re-check run ownership, stage bytes, re-prove digest and cited paths inside the lock, atomic rename. |
| `failProfiling`, `markProfileStale` | `src/daemon/project-profile.ts:651-703` | Preserve current and record an exact failure/staleness reason. `stale` remains readable/in use. |
| daemon validation | `src/daemon/project-profile-validate.ts` | The daemon validates identity and cited paths, rejects silent empty profiles, resolves path containment, and remains the acceptance authority. Provider/model identity is authored by the daemon envelope from the run (F2), not trusted from model output. |

### Foundation + F1/F2 contract (landed — completed)

Slice 1 foundation (`3832ea6`) plus follow-ups **F1** (bounded inventory) and **F2** (candidate/envelope split and requester/guidance provenance) are **on `main`**. They are completed contract, not open work. Remaining packages start at **P1**.

**Foundation concurrency and acceptance (keep green; do not reimplement in the coordinator).** All `state.json` read-modify-writes use `withProfileLock` over the hardened cross-process `withFileLock`. `beginProfiling(root, profiler, {ifIdle:true})` performs the check, content inventory, and state write under that lock. The coordinator must always use that overload for normal first/refresh work. `readProfileState` first reconciles a matching accepted `current.json` left behind by a crash, so a live durable run record maps to coalesced/pending, never a new launch. The default overload supersedes and is reserved for a run P2 has positively proved expired/dead; the foundation intentionally implements no expiry policy. Submission validates expensive inputs outside the lock, re-checks ownership, stages the candidate, and calls `proveProfileStillHolds` inside the lock before the final rename. That guarantee is “citations observed valid by the final pre-commit proof,” not “valid now”: the proof-to-rename instant cannot be closed because repo writers do not take `profile.lock`. P7 owns immediate post-commit drift detection. Validation also resolves command/workspace containment through real paths.

**F1 — safe bounded inventory (done).** `computeProfileInventory` content-hashes allowed regular files and:

- skips dependency/build segments (e.g. `.git`, `.hive`, `node_modules`, `vendor`, `dist`, `.venv`, …) and credential-store segments (e.g. `.ssh`, `.aws`, `.gnupg`, …);
- skips known-secret basenames/patterns (`.env` / `.env.*` except examples, private keys, credential files, …);
- normalizes to POSIX repo-relative paths, rejects absolute/`..` paths, uses `lstat`, never dereferences symlinks for content;
- fails closed with `ProfileInventoryLimitError` / `inventory-limit` after file-count, hashed-byte, path-segment, or wall-clock caps rather than returning a silently partial digest.

**F2 — candidate vs envelope and request provenance (done).** `ProjectProfileCandidateSchema` (`src/schemas/project-profile.ts`) holds only model-authored claims (`docs.briefable` and `conventionFiles` remain evidenced claim objects). `submitProfile` assembles the envelope from the authenticated active run and daemon clock: schema version, timestamps, project UUID, input digest, provider/model/agent, run id, tool session, and request provenance. The model cannot author those metadata fields. Request state carries requesters with in-lock timestamps and optional guidance (UTF-8, line-ending normalize only, 4 KiB cap) copied into accepted-profile provenance — an instruction to investigate, never a validation override.
...
P1 consumers must preserve the foundation's availability semantics. `profile_read` delegates to `readCurrentProfile` and returns profile-or-null; missing, malformed, or otherwise unreadable `current.json` is `null`, never a thrown parse error (`src/daemon/project-profile.ts:254-265`). `profile_status.hasCurrent` and the spawn gate are derived from that validated read, not from lifecycle text alone. A `stale` state with a readable current profile means “profile remains in use; refresh due”: status exposes both facts, `profile_read` returns it, and the validated-profile gate remains open. Only no current profile plus no session bypass closes the first-profile spawn gate.
...
`profile_submit` accepts `ProjectProfileCandidate`, never a full `ProjectProfile`. The handler combines the candidate with the authenticated active run: run/project/input digest and provider/model/agent come from `state.json`, the tool session comes from the observed provider process, and timestamps/request provenance come from daemon state. Return either `{status:"accepted"}` or `{status:"rejected", lifecycle, rejections: {code, message, at?}[]}` without collapsing the daemon's errors. `missing-unknowns` is repairable: a repo with no supported claims must submit at least one explicit `{subject, why}` unknown; a completely silent candidate is refused. Provider/model mismatch remains an `unauthorized` defense-in-depth rejection even though the candidate wire has no such fields.
...
P2 must expose this as an explicit start result, not a read-then-begin protocol: call `beginProfiling(root, profiler, {ifIdle:true})` and map a handle to `started`, `null` to `coalesced/pending`. A coalesced result mints no second credential, reservation, runtime directory, or provider process. `null` is never a crash ghost because `readProfileState` heals an accepted run whose state write was lost; it is a durable active run that the coordinator joins. The foundation does not expire runs. P2 owns a separate recovery/expiry decision: only after the recorded run deadline has elapsed **and** provider process/session liveness is positively absent may it invoke default `beginProfiling(root, profiler)` to supersede. Unknown/unreadable liveness is not proof of death. Test that ordinary concurrency never reaches the superseding overload, while one expiry-proven takeover does and the old run can no longer submit.
...
Add `buildProfilerPrompt` beside the launcher, separate from ordinary `buildAgentPrompt` at `src/daemon/spawner-impl.ts:641-695`. It carries project display name, run ID, profile candidate instructions (including evidenced `docs.briefable` and `conventionFiles` claims), optional guidance verbatim, and these explicit rules:

> You are Hive's project profiler. Inspect the project only with `profile_inventory`; do not use native file, shell, code execution, network, browser, or other MCP tools. Do not create or modify files, branches, worktrees, commits, dependencies, configuration, memory, or skills. Do not run build, test, lint, typecheck, install, package-manager, VCS, or project commands. Every briefable-doc and convention-file claim must include evidence and confidence; use convention `kind` to state what loads it, never infer that claim from its filename alone. Record explicit unknowns instead of guessing; even an empty repo must explain its unknowns. Submit only through `profile_submit`, repair validation errors if returned, and stop after acceptance.
...
The coordinator owns one in-memory job/pending slot and the foundation owns the durable run lock. The accepted profile also owns an authored relevance signal and an explicit freshness limit: `ProjectProfileStalenessSchema` at `src/schemas/project-profile.ts:228-261` says citations were observed valid by the final pre-commit proof, not that they remain valid now. Foundation validation enumerates and proves cited paths at `src/daemon/project-profile-validate.ts:397-518`; P7 must drive a current profile to `stale` through `markProfileStale` (`src/daemon/project-profile.ts:676-703`) when a cited path or `staleness.paths` input is observed missing or changed. Required behavior:

- Extract one pure `collectProfileCitations(profile): {path, kind}[]` helper from the validator's existing enumeration and use it in both validation and P7 so the lists cannot drift. It includes workspace paths; command `cwd`s; entry points; convention files; primary/briefable docs; every evidence path on languages, package managers, build systems, workspaces, commands, entry points, conventions, and docs; plus `staleness.paths`. Preserve the validator's `file | directory | any` kind, normalize/deduplicate repo-relative path/kind pairs once, and never reconstruct citations by scanning arbitrary strings in the profile.
- At every daemon session start, compute the canonical whole-safe-inventory content digest and compare it with the accepted profile's `project.inputDigest`, and verify every collected citation still exists with its validated kind/containment. This is the broad correctness backstop required by the epic: if equal and every citation still holds, launch no profiler; a same-size edit must change it. Any digest mismatch or missing/changed citation calls `markProfileStale(root, "session-start", reason)` before scheduling refresh, even when the path was not model-authored as a staleness input.
- Treat `staleness.paths` and all cited paths as immediate signals. A filesystem or landed-path event matching a cited file, a cited directory/descendant, or a `staleness.paths` entry calls `markProfileStale` immediately and bypasses the 2-second debounce. Use repo-relative reasons (`Cited path missing: <path>` or `Cited path changed: <path>`) and sources `filesystem`, `landing`, `session-start`, or `post-submit`; never leak absolute paths. Other safe-inventory events still mark stale and reset the whole-digest debounce. A session-start or post-submit check whose digest is unchanged and whose citations all hold launches no profiler.
- Keep the watcher active during profiling and buffer its normalized changed paths with a monotonic sequence. On acceptance, install the new cited-path set, replay events observed since that run began, then immediately recompute the safe inventory digest and citation existence against the accepted profile. If the repo moved in the foundation's proof-to-rename instant or immediately after commit, call `markProfileStale(root, "post-submit", reason)`. This closes the observation gap without pretending `profile.lock` controls repo writers; `current.json` remains readable/in use while the replacement is scheduled.
...
- `src/adapters/brief.ts:25-32`: construct the exact allowlist as `profile.docs.briefable.map((claim) => claim.path)` and derive the primary-doc selector from `profile.docs.primary?.path`. The claim evidence/confidence remains available for diagnostics but is not dumped into the brief. Set `briefableDirectories` empty rather than widening authored authority.
...
- `src/daemon/spawner-impl.ts:510-586`, `:1922-1942`: render all applicable test/typecheck/validate commands with their `cwd` and workspace scope. Do not silently choose the first monorepo command. Match `conventionFiles` to the provider adapter by the validated `kind` (`agents | claude | other`), never by basename; adapter tests must prove the exact claimed path is the one loaded or referenced, and `other` is only a generic conventions pointer unless that adapter explicitly supports it. When fields are absent/bypassed, use generic wording and do not invent `bun test` or a convention filename.
...
|---|---|---|---|---|
| F1 | ~~Safe bounded inventory~~ **LANDED** | `src/daemon/project-profile.ts` inventory path + inventory tests | foundation | same-size edit changes digest; secrets/symlinks/binaries/caps covered |
| F2 | ~~Candidate/envelope + provenance~~ **LANDED** | `src/schemas/project-profile.ts` candidate/request; `submitProfile` envelope assembly | foundation | model cannot author run/provider/model metadata; guidance persists; spoofed provenance rejects |
| P1 | Capability role/actions, wire schemas, `ProfileControl` seam, five MCP tools, internal gate/bypass routes | `src/daemon/capabilities.ts:19-132`, `src/daemon/server.ts:938-953`, `:2375-2406`, `:3267-3914`, `src/daemon/credentials.ts:29-83`, auth docs/tests | F1 + F2 (landed) | exact role matrix/tool visibility; cross-project/completed token denied (opaque, id-free refusal — a foreigner learns nothing, not even the lifecycle); all rejection codes are lossless; unreadable current is null; stale current remains readable and gate-valid. P1 owns the inventory **wire contract, tool, auth, and role-gated visibility only**; the catalog/content **caps** (500-entry/256 KiB page; 32-path, 256 KiB/file, 512 KiB/response, 8 MiB/run content) live behind `ProfileControl` and are **reassigned to P3** — see that row |
| P2A | Specialized launcher core, shared routing/admission extraction, prompt, isolated runtime, run credential, shipped skill | `src/daemon/project-profile.ts:377-440`; `src/daemon/spawner-impl.ts:1417-2138`; new launcher; `src/skills/shipped.ts:17-51`; skill packaging | P1 | no worktree/branch/repo cwd; prompt contains all rules; `ifIdle` null coalesces without launch; only expiry-proven dead run uses superseding overload |
...
| P3 | `ProfileCoordinator`, daemon assembly, job lifecycle, capacity/deferred state | `src/daemon/server.ts:419-628`, `:963-1102`, `src/cli/daemon.ts:53-210`, new coordinator | F2 + P2B + P2C | one run under concurrent requests; first failure exact; current survives failed refresh; clean shutdown revokes token/session. As the production `ProfileControl`, **enforces the inventory catalog/content caps reassigned from P1** (page/byte/32-path/8 MiB-per-run) and **test-proves every cap before any production wiring**; run binding, cursor↔`{projectUuid,runId,inputDigest}`, and the opaque-refusal contract are verified with P1's shared `assertProfileControlOpacity` conformance, which **requires both** scenarios it enforces — (a) active run + foreign subject and (b) completed/no-active run + former-owner subject — so a stale token is never answered with `denied/no-active-run` (the binding check must precede any operational code) |
...
| P7 | Session digest, cited-path/post-submit freshness, filesystem debounce, landing immediate checks, capacity deferral | coordinator; cited-path collector at `src/daemon/project-profile-validate.ts:397-518`; `markProfileStale` at `src/daemon/project-profile.ts:676-703`; `src/daemon/landing.ts:39-42`, `:477-532`; `src/daemon/server.ts:1485-1620`, `:2174-2222` | F1 + P3 | unchanged start launches zero; cited path deleted/changed during or after commit becomes stale immediately while current stays readable; same-size edit refreshes; event storm coalesces; scarce capacity defers |
| P8 | Validated-profile consumers and generic fallback; remove remaining old readers | `src/adapters/brief.ts:25-32`; `src/cli/orchestrator.ts:225-234`; `src/daemon/spawner-impl.ts:510-586`, `:1874-1942`; legacy `src/adapters/profile.ts` callers/tests | F2 + P4 | briefable claim paths, convention kinds, and scoped commands are consumed exactly; bypass/null gets generic prompt; no runtime import of legacy profile generator |

**Sequencing starts at P1.** F1 and F2 are landed and are not open work. P2A and P2C independently follow P1; P2B follows P2A; P3 waits for P2B and P2C. P4, P5, and P7 may be assigned in parallel after P3. P4 and P7 touch disjoint regions of `src/daemon/server.ts`, but Hive lands fast-forward-only: whichever lands second must rebase onto the first and rerun tests/typecheck before review/landing. P6 and P8 may proceed in parallel after P4. The landed cross-ecosystem fixtures in `src/adapters/project-fixtures.test-support.ts` should be consumed by P2B, P7, and P8 rather than copied.
...
- foundation contract: keep Erica's four-way in-process/cross-process `ifIdle`, missing-unknowns, commit-time deletion, crash reconciliation, and symlinked-cwd tests green; keep F1 inventory-limit/secret/symlink tests and F2 candidate-envelope tests (model cannot author run/provider/model metadata) green;
...
- landing/drift: `src/daemon/landing.test.ts` and coordinator fake-clock tests, including same-size writes, ignored paths, overflow, event storms, cited-file deletion/change after acceptance, a cited-path event buffered across the proof-to-rename window, immediate `markProfileStale` source/reason, and readable current while stale;
...
- end-to-end: empty repo with explicit unknowns, silent-empty rejection/repair, evidenced briefable/convention claims, Bun/TypeScript, Cargo workspace, Python monorepo, mixed ecosystem, internal/outside symlinks, tracked secret file, huge-inventory failure, provider routing refusal, invalid-then-valid submit, crash between current/state writes, two simultaneous launchers, bypass restart.
...
8. **Two-file atomicity.** Atomic rename protects each file, not the pair. `readProfileState` logically reconciles `profiling` to `current` when `current.json` carries the same run ID (`src/daemon/project-profile.ts:271-335`), so `{ifIdle:true}` cannot coalesce forever onto that crash ghost. All middle-slice readers must use this API rather than parse `state.json` directly, and the kill-between-writes regression remains load-bearing.
9. **Proof-to-rename/post-commit freshness.** The final proof at `src/daemon/project-profile.ts:533-569` deliberately leaves one rename-sized window and cannot control later repo writes. P7's continuous watcher, buffered-event replay, immediate post-accept digest/citation check, and direct `markProfileStale` call are therefore correctness, not optimization. Tests must force the mutation into that window; a session-start-only check is insufficient because it silently trusts the profile for the rest of the current session.
10. **Guidance sensitivity.** User guidance becomes local provenance and provider prompt content. Disclosure/help should state that fact, cap it, and never echo it into ordinary agent prompts unless the accepted schema intentionally exposes it.
11. **Project-registry lost update outside profile locking.** Landed `3832ea6` makes the registry temp filename process-private at `src/daemon/project-identity.ts:51-60`, fixing concurrent rename `ENOENT`, but registry persistence remains last-write-wins across processes. Concurrent first identity resolution for two different projects in one `HIVE_HOME` can lose one entry. The profile coordinator resolves its one project identity before scheduling and does not attempt to fix this broader registry ownership issue; it needs a separate project-identity owner and cross-process read-modify-write lock.
