...
| P1 | Capability role/actions, wire schemas, `ProfileControl` seam, five MCP tools, internal gate/bypass routes | `src/daemon/capabilities.ts:19-132`, `src/daemon/server.ts:938-953`, `:2375-2406`, `:3267-3914`, `src/daemon/credentials.ts:29-83`, auth docs/tests | F1 + F2 (landed) | exact role matrix/tool visibility; cross-project/completed token denied; all rejection codes are lossless; unreadable current is null; stale current remains readable and gate-valid |
