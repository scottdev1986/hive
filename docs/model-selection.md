...
**Capability floors** (`~/.hive/routing.toml`, table `[floors]`) are a second kind of user policy, checked against the candidate every one of the three rungs above produces — applied in the ruled order pin → capability floor → user policy → benchmark ordering → quota. A floor is an explicit per-vendor allowlist naming the models that clear the bar for building work (the `deep`, `standard`, and `review` tiers; `cheap` is exempt by design — the user's own rule is that the simplest work goes to haiku-class models). Hive never invents a quality ranking to enforce it (no-model-judgment ruling): it only matches ids the user wrote against what a rung resolved. A candidate the floor excludes is dropped — the pin included, because standing rule A ("nothing pushes a route below it") is unconditional — and derivation falls through to the next rung; when every rung's candidate fails the floor, the cell REFUSES, naming the floor and the excluded id(s), never a silent downshift to whatever a vendor happened to default to. This landed 2026-07-12 after a building task was twice routed to a model the user did not consider acceptable for that work, with no floor anywhere the router could read.

**The mechanism is durable; the values are not, and none ship pre-filled.** The binary carries the schema (`RoutingFloorsSchema`, `src/schemas/routing-derivation.ts`) and the enforcement only — with no `[floors]` table, derivation runs exactly as it did before floors existed, and that is the out-of-the-box state: nothing is seeded. Which models actually clear a floor is the user's own, current, editable call, expected to change as the model lineup does — it is written once in `routing.toml` and nowhere else, never inferred from a name, price, or release date. The format, illustrated with placeholder names (not a recommendation — pick your own, and expect to revisit them):

```toml
[floors.claude]
allow = ["vendor-model-a", "vendor-model-b"]
note = "why this floor, for your own future reference"  # optional

[floors.codex]
allow = ["vendor-model-c"]
```

— membership, never a rank. `note` is carried through unread by the enforcement logic; it exists only so the user can record their own reasoning next to the values.
