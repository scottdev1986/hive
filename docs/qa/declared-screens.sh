...
# Prints the declared slugs, one per line. Fails loudly, and never returns an
...
qa_declared_screens() {
  local binary="$1" corpus="$2" output slugs count claimed
  if [ ! -x "$binary" ]; then
  ...
  slugs="$(printf '%s\n' "$output" | sed -n 's/^SHELL-SCREEN \([^|]*\)|.*/\1/p')"
