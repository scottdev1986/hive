...
- **Codex-internal subagents are distinct execution identities.** `codex features list` reports `multi_agent` as a stable feature on by default; a worker that spawns internal children gives them identities Hive never authorized, reserved quota for, or attested (the incident's `/root/review` and `/root/review_grok` rollouts, which run at their own cwd). Hive disables them with `-c features.multi_agent=false` on every TUI spawn/resume (`src/adapters/tools/codex.ts`) and on the app-server host argv (`src/adapters/tools/codex-app-server.ts`). There is no SubagentStart/SubagentStop backstop in Hive. Codex **writers** are refused at launch until an enforceable per-mutation boundary exists; read-only Codex (review/query/root) remains available.
- **Fail-closed is non-destructive.** A Codex writer observed to have drifted is paused, not killed: capability is revoked first, then a native turn interrupt plus a SIGSTOP of the process tree freezes it while preserving the process, rollout/thread, tmux holder, and toolSession (`pauseWriterForIdentityDrift`, `src/daemon/teardown.ts` `suspendCapturedTree`). `hive_resume` reattests and only returns write authority (reissued at a fresh capability epoch, same process SIGCONT'd) once the observed identity matches the launch identity again. A suspended process cannot acknowledge, so the pause is measured by process/daemon state, never an ACK.
