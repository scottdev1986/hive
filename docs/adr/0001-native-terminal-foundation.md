...
- The PTY child is a conventional interactive login zsh (`/bin/zsh -l -i`). The Hive TUI (`hive agent-ui`, powered by OpenTUI) runs as a program inside that zsh. When the user exits the TUI, they land in the interactive zsh — the same session (cwd, history, jobs). The pane does not die when the TUI exits; it dies when the user closes the pane/window.
- Vendors (Claude, Codex, Grok, Kimi, OpenCode) run as headless protocol adapters on pipes (ACP/stdin/stdout), not as their own TUIs on the PTY. Hive never reconstructs a transcript to imitate vendor TUIs and never treats terminal-screen text as authoritative agent state.
