/** The release manifest: the one document `hive update` trusts. Two independent signatures protect a release, and neither substitutes for the other. Apple's Developer ID signature authenticates the executable to macOS. The Ed25519 signature over this manifest authenticates *update policy* to Hive: which version is current, which bytes are it, whether it is a security release. A notarized binary served from the wrong manifest is still the wrong update. The offline Ed25519 key exists and every release since 0.0.6 is signed with it: the public half is inlined into each binary at build time, the private half lives only as a CI secret and touches only `scripts/signing/sign-manifest.ts`. Verification is therefore mandatory and fail-closed — a build that knows the key refuses a manifest that is unsigned, altered, or signed by anything else. A stripped `.sig` is a refusal, not a downgrade, because the alternative is an attacker choosing our verification policy for us. A build with *no* key embedded (a source checkout, or a release from before 0.0.6) still takes the unsigned branch and says so out loud rather than implying a signature it never checked. That branch must never be reachable from a current release, and `hive update status` names which one you are on. `HIVE_RELEASE_PUBLIC_KEY` is a *list*, comma-separated, and that is what makes rotation possible without a flag day: ship a release trusting {old, new}, wait for it to propagate, then sign with new and drop old. Verification accepts a signature from any listed key, so no single release has to be the one where every installation either updates or bricks. It buys nothing against a *compromised* key — an attacker holding the old private half can still sign for binaries that still list it, and recovery requires an out-of-band reinstall. */
import {
  createHash,
  createPublicKey,
