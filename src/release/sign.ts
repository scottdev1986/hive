/** Developer ID signing and Apple notarization for the release artifacts. This is the second of a release's two independent signatures. Apple's Developer ID signature authenticates the executable to macOS so Gatekeeper does not quarantine it; the Ed25519 manifest signature (see manifest.ts) authenticates *update policy* to Hive. Neither substitutes for the other, and this module owns only the first. Everything here is gated on `signingConfigFromEnv` returning non-null. When the CI secrets are absent it returns null, `build.ts` skips every call below, and the pipeline publishes unsigned artifacts — no flag to flip, no second code path to forget. Hive's own repository has the secrets set, so its releases take the signed path; the unsigned branch is what a fork gets, and it degrades to a working build rather than a broken one. Two facts were verified against real tool behaviour on macOS, not memory, and both are load-bearing: 1. `bun build --compile` writes its own ad-hoc "linker-signed" signature, and on Bun >= 1.3.12 that signature reserves too little space in __LINKEDIT for a real one. Re-signing on top of it produces a truncated signature that fails `codesign --verify --strict` with "main executable failed strict validation". The fix is to build with `BUN_NO_CODESIGN_MACHO_BINARY=1` (the workflow sets it only when signing is on) so codesign can lay down a correct signature. Proven locally: the no-codesign build signs strict-clean on both arm64 and cross-compiled x64; the default build does not. 2. A standalone Mach-O cannot have a notarization ticket stapled — `stapler` and notarytool accept only .zip/.pkg/.dmg and staple only bundles. The CLI slices are therefore notarized inside a zip (which registers their cdhash with Apple) and rely on Gatekeeper's online ticket lookup; only the .app is stapled and works offline. */
