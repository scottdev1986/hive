/** The versioning contract, as executable code. One push to `main` bumps the patch by exactly one, whatever the push contains. A hundred local commits arriving in a single push produce one release, because the bump is a function of the *tip commit* and the existing tags — never of the commit count. Idempotency comes from one rule: a commit that already carries a release tag is never released again. Re-running a workflow, re-pushing the same tip, or a force-push that lands nothing new all resolve to `skip`. Publishing the tag is a compare-and-swap against the remote (`git push` of a new ref fails if the ref exists), so two concurrent runs cannot both mint the same version. Hive is 0.0.x, patch-only, on purpose. We are a long way from deciding what a minor bump would mean, and a scheme with one moving part cannot drift. A conventional-commits scheme was the alternative: it reads intent from commit messages, which makes the version a function of prose that nobody proofreads, and it would silently mint 0.1.0 the first time someone typed `feat:`. This module refuses any tag outside the series rather than guess what it meant. */
