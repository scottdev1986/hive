...
describe("the routing surface prints only what it derived", () => {
  ...
  const rendered = (discovery: Record<"claude" | "codex", ProviderDiscovery>) =>
    formatDerivedRouting(derive(discovery), NOW);
  ...
  test("a value no layer could author prints as unknown, never as a guess", () => {
    ...
    // printing any model as though one had been derived — there is no shipped
    // table left to smuggle one in from.
    expect(output).toContain("claude: UNAVAILABLE (claude is not signed in)");
    // Codex's effort was really derived (cheap=low, advertised), so it prints
    // its value AND its source.
    expect(output).toContain("tier effort policy");
    ...
    // Claude's cells print the em dash and the refusal, never a model or a
    // `medium` that no vendor surface ever recommended.
    expect(output).toContain("—");
    expect(output).toContain("REFUSED/unknown");
  });

  test("the header says it governs, and warnings name every refusal", () => {
    const output = rendered({ claude: down("down"), codex: CODEX_UP });
    // There is no non-governing mode left to describe: this IS what spawns
    // launch, and the header owns that instead of hedging.
    expect(output).toContain("GOVERNING");
    expect(output).toContain("This is what live spawns launch");
    expect(output).toContain("routing.toml");
    // Every cell nothing could author is a loud warning naming the remedy.
