...
/** `GET /orchestrator-status` with the user credential: independently measured root identity, exact turn state, and terminal lifecycle. */
  ...
  try {
    while (signal?.aborted !== true) {
      try {
        ...
        // Root provider status and terminal lifecycle ride the same line. A
        // failed read omits the whole root snapshot; a successful read always
        // carries either the exact turn or a concrete connection state.
