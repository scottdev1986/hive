...
/** `hive routing policy` / `set-provider` / `set-model` / `set-effort` / `set-route` / `export` — the Model Control Center's contract. The UI is a separate AppKit process that shells out to these commands; every read and write goes through the daemon (the store's sole writer), and every mutation carries the revision the caller read, so concurrent edits conflict loudly instead of clobbering. Enablement here IS consent to spend, so these commands are a safety surface: they validate locally, the daemon validates again, and success prints the full updated document — the UI never has to guess what state it produced. */
...
async function fetchPolicy(port: number): Promise<RoutingPolicy> {
  ...
  }).json("/routing/policy", undefined, "throw");
...
/** One route candidate: `provider/model` (effort provider-controlled, weight 1), `provider/model@LEVEL` (exact effort), `provider/model@none` (the vendor's stated no-effort axis), each optionally suffixed `=WEIGHT` with an integer 1–100. The model is always a specific id — there is deliberately no way to write "whatever the vendor picks", and a bare "default" model id is rejected downstream by the schema. */
...
/** Deterministic dump: stable key and row order, byte-identical for identical policy — the inspectability half of the SQLite ruling. */
...
/** Replace one scope's route. Zero candidates clears the scope back to unconfigured (`global` cleared refuses automatic routing). */
