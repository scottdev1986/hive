/** The landing gate's git: a fast-forward-only merge of a writer's branch into the primary checkout, and — the part this module exists for — an honest account of what happened when that does not work. The bug that motivated all of it: `git merge --ff-only` was being run with a 30-second deadline, and a helper reported *any* failure as `git merge timed out after 30000ms`. It had not timed out. Bun sets `Subprocess.killed` to true for a process that exits on its own — including a normal, instant, non-zero exit — so the timeout branch fired on every failing git command, and the real stderr, which said exactly what was wrong, was thrown away and replaced with a fabricated stopwatch reading. A blocked merge fails in about 9ms. Nobody was ever waiting 30 seconds; they were being lied to in 9 milliseconds and then told it took 30 seconds. So: the timeout is tracked by an explicit flag we set ourselves, git's own stderr is never discarded, and every way a landing can fail names its cause and, when a user has to decide, says what to do in one labeled `Fix:` line. What this module will never do is *resolve* a landing failure on its own. Every blocker here is one of two things: work only the writer can redo (a rebase invalidates the tests it just ran green, so Hive cannot rebase for it), or a change in the user's working tree that Hive did not write and cannot prove is disposable. Guessing at either is how you lose someone's work. One exception, precisely because it is provable: an untracked file in the primary whose bytes are identical (by blob hash) to the version the branch commits at the same path. Removing it loses nothing — the fast-forward immediately restores the same content, tracked. This is the most ordinary collision Hive has: a user drops a file in, an agent copies it into its worktree and commits it, and git then refuses to fast-forward the primary over the user's original. When the bytes differ, that proof is gone and the usual rule holds absolutely: name both versions and let the user choose. Two invariants complete the honesty rule, both learned from one 2026-08-13 incident. First, the target is a branch, never a position: a detached primary checkout has no landing target, so `resolveLandingTargetBranch` refuses with the detachment named rather than hand downstream code the literal string "HEAD" to reason about — a fallback that once produced a written instruction to `git rebase HEAD` onto another agent's unlanded tip. Second, a landing grant authorizes one branch's OWN commits, not whatever the tip's ancestry contains: every commit the merge would carry that another ref still claims as unlanded work refuses the land, and every commit a land does carry is named in the returned receipt. Authorization is per branch name; its effect must never be the branch tip's whole ancestry. */
