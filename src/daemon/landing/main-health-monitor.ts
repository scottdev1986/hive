...
/**
 * How long a dirty checkout is given to come clean before the probe declines,
 * and how often it is re-read while waiting. A landing writes to the checkout
 * and a check is triggered by a landing, so the check can arrive while a write
 * is still in flight; such a write is finished moments later. Work somebody
 * left uncommitted does not clear itself. Waiting separates the two without
 * having to guess which one it is looking at, and the wait is bounded so a
 * checkout that really is dirty is still reported rather than parked on.
 */
...
/**
 * Thrown instead of returning a MainHealthResult when the suite itself never
 * ran — as opposed to running and reporting real failures. The monitor's
 * caller already treats a thrown error from a health check as "the check
 * itself misbehaved" (logged locally, no RED MAIN alert); this reuses that
 * path instead of reporting main as red for a problem that is not main's.
 */
export class SuiteNeverStartedError extends Error {}
...
const readTail = async (
  ...
  return `${tail}${decoder.decode()}`.slice(-OUTPUT_LIMIT_BYTES);
};

const scratchKilobytes = async (scratch: string): Promise<number> => {
  const child = Bun.spawn(["du", "-sk", scratch], {
    stdout: "pipe",
    stderr: "ignore",
  });
...
const readDirtyPaths = async (repoRoot: string): Promise<string[]> => {
  const child = Bun.spawn(["git", "-C", repoRoot, "status", "--porcelain"], {
  ...
  if (exitCode !== 0) {
    throw new Error(
      `cannot inspect primary checkout: ${stderr.trim() || `git exited ${exitCode}`}`,
    );
  }
...
/**
 * Waits for the checkout to be clean, up to `settleWaitMs`, and throws naming
 * what was still there if it never is. A caller's abort ends the wait at once
 * so stopping the daemon never has to sit through the budget.
 */
const assertPrimaryCheckoutIsClean = async (
  repoRoot: string,
  signal: AbortSignal,
  settleWaitMs: number,
): Promise<void> => {
  const deadline = Bun.nanoseconds() + settleWaitMs * 1_000_000;
  let paths = await readDirtyPaths(repoRoot);
  while (paths.length > 0 && !signal.aborted && Bun.nanoseconds() < deadline) {
    await Bun.sleep(SETTLE_POLL_MS);
    paths = await readDirtyPaths(repoRoot);
  }
  if (paths.length === 0) return;
  throw new PrimaryCheckoutDirtyError({ paths, settleWaitMs });
};

const summaryCount = (output: string, word: string): number => {
  const match = output.match(new RegExp(`^\\s*(\\d+)\\s+${word}\\b`, "m"));
  const digits = match?.[1];
  return digits === undefined ? 0 : Number.parseInt(digits, 10);
};

/**
 * True when bun's own end-of-run summary shows that no test body ever
 * executed — as opposed to test bodies running and some of them failing.
 * bun reports a file that never loaded (a throwing preload, a missing
 * import, any setup failure) as one "error" per file, with no "(pass)" or
 * "(fail) <name>" line for any test inside it, so a run where every
 * recorded failure is one of those between-test errors — zero passes, zero
 * skips, and no fail beyond what "errors" already accounts for — never
 * reached a single test. This reads bun's stable summary line, not any
 * particular guard's error text, so it keeps working if that text changes.
 */
const suiteNeverStarted = (output: string): boolean => {
  const pass = summaryCount(output, "pass");
  const skip = summaryCount(output, "skip");
  const fail = summaryCount(output, "fail");
  const errors = summaryCount(output, "errors?");
  return pass === 0 && skip === 0 && fail <= errors;
};

export const runMainBunTests = async (
  repoRoot: string,
  signal: AbortSignal,
  settleWaitMs: number = SETTLE_WAIT_MS,
): Promise<MainHealthResult> => {
  // Reports name a branch ref, not an arbitrary worktree snapshot. A detached
  // checkout would need its own node_modules installed per run, or a shared
  // dependency directory that is not the committed tree, as well as setup and
  // disk alongside the bounded scratch root. Instead, this read-only porcelain
  // check declines dirty checkouts, including untracked paths, before or after
  // the suite; a mid-run change made the result a measurement of a tree that
  // no longer exists. Waiting for the checkout to settle keeps that guarantee:
  // what would invalidate a run is work somebody left in the tree, and that is
  // exactly what does not clear while the probe watches.
  await assertPrimaryCheckoutIsClean(repoRoot, signal, settleWaitMs);
  const scratch = await mkdtemp(join(tmpdir(), "hive-main-health-"));
  try {
    await mkdir(join(scratch, "bun-cache"));
    let timedOut = false;
    // The bounded test root is mandatory (test/test-root-preload.ts refuses
    // a bare `bun test`); this is the same entry point package.json's own
    // "test" script and every other in-repo caller use to run the Bun suite
    // under it, without also pulling in the separate native sessiond suite
    // that script chains after it.
    const child = Bun.spawn(
      ["bun", "run", "scripts/test-sandbox.ts", "--", "bun", "test"],
      {
        cwd: repoRoot,
        env: {
          ...process.env,
    ...
    let scratchLimitExceeded = false;
    let scratchMeasurement: Promise<void> | null = null;
    const kill = () => {
      try {
    ...
    const scratchWatch = setInterval(() => {
      if (scratchMeasurement !== null) return;
      scratchMeasurement = scratchKilobytes(scratch)
        .then((kilobytes) => {
          if (kilobytes <= SCRATCH_LIMIT_KB) return;
          scratchLimitExceeded = true;
          kill();
        })
        .finally(() => {
          scratchMeasurement = null;
        });
    }, SCRATCH_CHECK_MS);
    try {
      const [exitCode, stdout, stderr] = await Promise.all([
        child.exited,
        readTail(child.stdout),
        readTail(child.stderr),
      ]);
      const detail = `${stdout}${stderr}`.trim();
      await assertPrimaryCheckoutIsClean(repoRoot, signal, settleWaitMs);
      // A caller-requested abort (main moved, or the monitor is stopping)
      // also ends the run with no test body executed; that is not evidence
      // of a broken suite, so it is excluded here the same as a timeout.
      if (
        !signal.aborted &&
        !timedOut &&
        !scratchLimitExceeded &&
        suiteNeverStarted(detail)
      ) {
        throw new SuiteNeverStartedError(
          `bun test never executed a test body:\n${detail}`,
        );
      }
      return {
        ok: exitCode === 0 && !timedOut && !scratchLimitExceeded,
        detail: scratchLimitExceeded
          ? `bun test scratch exceeded ${SCRATCH_LIMIT_KB} KiB\n${detail}`
          : timedOut
            ? `bun test exceeded ${TEST_TIMEOUT_MS}ms\n${detail}`
            : detail,
      };
    } finally {
      clearTimeout(timeout);
      clearInterval(scratchWatch);
      signal.removeEventListener("abort", onAbort);
      await scratchMeasurement;
    }
  } finally {
