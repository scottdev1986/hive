...
import type { FsEvidence, VolumeBehavior } from "./project-identity-types";
...
export function evidenceMatches(a: FsEvidence, b: FsEvidence): boolean {
  // st_dev identifies the current mount, not the filesystem durably: macOS can assign a different value after a reboot while the directory's inode and birth time remain unchanged. Treating that remount as replacement locks every registered project behind NEEDS_SETUP. The inode + birth-time pair still refuses a recreated directory; dev remains useful for process-local volume behavior and diagnostics, but is not persisted identity evidence.
  return a.ino === b.ino && a.birthtimeMs === b.birthtimeMs;
}

/** Step 1 of the resolver: canonicalize the *existing* invocation directory using the physical path and the volume's case behavior. realpath(2) resolves symlinks, the /tmp -> /private/tmp style firmlinks, and -- measured, though not documented by Apple -- rewrites each component to its on-disk case and Unicode normalization on volumes that fold those distinctions. That is why `identityKey` folding is defense in depth rather than the mechanism. */
