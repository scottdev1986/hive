...
import { daemonLogPath } from "./daemon-log";
...
export async function daemonInstanceLiveness(
  ...
  if (portEvidence.state !== "valid" || portEvidence.value > 65_535)
    return "unknown";
...
export async function probeDaemonReuse(
  ...
  try {
    ...
    const reason =
      actual === null
        ? "missing or malformed reuse handshake"
...
export async function ensureStarted(): Promise<number> {
  ...
  mkdirSync(resolve(getHiveHome(), "logs"), { recursive: true });
  const stderr = openSync(daemonLogPath(), "a", 0o600);
  const child = (() => {
    try {
      return Bun.spawn(daemonSpawnArgv(IS_RELEASE_BUILD, process.execPath), {
        cwd: process.cwd(),
        detached: true,
        env: {
          ...process.env,
