...
import { HandoffBundleSchema, isOrchestratorName } from "../schemas";
import { redactTerminalEvidence } from "./activity-snapshot";
...
async function git(cwd: string, args: string[]): Promise<string> {
  ...
  if (exitCode !== 0) {
    throw new Error(
      `git ${args[0] ?? "command"} failed: ${stderr.trim() || exitCode}`,
    );
  }
  return stdout.trim();
}
...
export async function buildHandoffBundle(
  ...
  const requirements = input.messages.filter(
    (message) =>
  ...
  const addressed = input.messages.filter(
    (message) => message.to === input.agent.name,
  );
  ...
  return HandoffBundleSchema.parse({
    ...
    requirementRefs: requirements.map((message) => ({
      kind: "message",
      id: message.id,
      retrieval: {
    ...
    messagesThrough: addressed.reduce(
      (through, message) => Math.max(through, message.sequence),
      0,
    ),
    ...
    activity: {
      ...
      // No provider currently exposes a universally executor-bound transcript
      // cursor. In particular, Kimi's wire.jsonl merges its TUI and kimi web.
