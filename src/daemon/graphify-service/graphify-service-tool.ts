...
export function registerGraphTool(
  ...
  // The mid-task half of the graph-first mandate: the same locate the spawn brief runs (Hive-side seeding + expansion over graph.json), callable with a natural-language question. Lives on Hive's server, not graphify's — that surface is pre-1.0 and not ours — and never blocks: every failure is an honest "use grep" answer, not an error.
  server.registerTool(
    "graph_locate",
    {
    ...
    async ({ question }, context) => {
      ...
      return toolResult(
        await graphLocate(deps.repoRoot, question, context.mcpReq.signal),
        "locate",
      );
    },
  );
