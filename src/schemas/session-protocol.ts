...
export const FRAME_TYPE_GROUPS = [
  ...
  {
    names: ["CREATE_BEGIN", "CREATE_INPUT", "CREATE_COMMIT", "CREATED"],
    direction: "daemon-broker-bidirectional",
    purpose: "transactional-create",
  },
  {
    names: ["LIST", "LISTED"],
    direction: "daemon-broker-bidirectional",
  ...
  {
    names: ["INPUT_ORPHAN_DISCARD", "ORPHAN_DISCARDED"],
    direction: "daemon-broker-host-bidirectional",
  ...
  {
    names: ["CLAIM_ACQUIRE", "CLAIM_RESULT"],
    direction: "viewer-host-bidirectional",
    purpose: "authoring-claim",
  },
  {
    names: ["HUMAN_INPUT", "CLAIM_RELEASE", "GESTURE_INPUT", "INPUT_SUBMIT"],
    direction: "viewer-to-host",
    purpose: "ordered-human-input",
  },
  {
    names: [
      "AUTOMATION_BEGIN",
  ...
  {
    names: ["HOST_REGISTER", "HOST_ADOPT", "GRANT_REGISTER"],
...
const SelectedProtocolSchema = z
  .strictObject({
...
const SessionExitSchema = z
  ...
  .nullable();
const SessionSurvivorsSchema = z
  .array(
    z.strictObject({
      pid: z.number().int().positive(),
      startToken: z.string().min(1),
...
export const SessionInspectionSchema = z
  .strictObject({
    ...
    input: z.strictObject({
      ...
      // arbiter. It is not an arbiter state and the state machine never emits it.
...
/** Correlated result union shared by INPUT_SUBMIT and RESIZE, plus the
...
const HelloCommonShape = {
  /** HELLO carries schemaVersion; the v1 shape fixes it to 1. */
  schemaVersion: z.literal(1),
  /** HELLO names the peer build hash; shape adjudicated as buildId. */
  buildId: z.string().min(1),
  /** HELLO names the Hive instance. */
  instanceId: z.string().min(1),
  /** HELLO names a same-major minor range; shape adjudicated as min/max minor. */
  protocol: z
    .strictObject({
      major: z.literal(SESSION_PROTOCOL_VERSION.major),
      minMinor: ProtocolMinorSchema,
...
const DaemonControlIdentitySchema = z
  .strictObject({
    ...
    wireProtocol: z
      ...
      .readonly(),
    /** src/daemon/handshake.ts DaemonHandshake.schemaEpoch is the schema identity
     * in terminal-stack-transition.html#security today; protocol/schema hashes
     * extend both later. */
...
export const HelloPayloadSchema = z
  .discriminatedUnion("clientRole", [
    z.strictObject({
      ...HelloCommonShape,
      /** Connect a viewer; shape adjudicated to the four connecting roles. */
      clientRole: z.literal("viewer"),
      /** A viewer may present a grant; shape adjudicated as optional only for viewers. */
      grantToken: z.string().min(1).optional(),
    }),
    z.strictObject({
      ...HelloCommonShape,
      /** terminal-stack-transition.html#security authenticates daemon control
       * against kernel identity and the existing daemon handshake. */
      clientRole: z.literal("daemon"),
      /** The six-way daemon comparison uses the existing handshake's exact field
       * names and semantics. */
      daemonControl: DaemonControlIdentitySchema,
    }),
    z.strictObject({
      ...HelloCommonShape,
      /** Broker peers authenticate by kernel credentials; they cannot present grants. */
      clientRole: z.literal("broker"),
    }),
    z.strictObject({
      ...HelloCommonShape,
      /** Host peers authenticate by kernel credentials; they cannot present grants. */
...
export const WelcomePayloadSchema = z
  .strictObject({
    /** WELCOME is a v1 control payload. */
    schemaVersion: z.literal(1),
    /** WELCOME returns the selected version. */
    protocol: SelectedProtocolSchema,
    /** WELCOME returns the endpoint instance ID. */
    instanceId: z.string().min(1),
    /** WELCOME names the endpoint; shape adjudicated to broker or host. */
    endpointRole: z.enum(["broker", "host"]),
    /** WELCOME returns the endpoint build ID. */
    buildId: z.string().min(1),
    /** WELCOME returns an engine ID; shape adjudicated nullable until bound. */
    engineBuildId: z.string().min(1).nullable(),
    /** WELCOME returns a connection ID; shape adjudicated as decimal uint64. */
...
export const ErrorPayloadSchema = z
  .strictObject({
    /** ERROR is a v1 control payload. */
    schemaVersion: z.literal(1),
    /** ERROR carries one typed code from the normative error table. */
    code: WireErrorCodeSchema,
    /** Error meanings are surfaced as a nonempty redacted message. */
    message: z.string().min(1),
    /** INTERNAL and verification failures may carry a diagnostic ID. */
    diagnosticId: z.string().min(1).nullable(),
  })
  .readonly();
...
const HostRecordProjectionSchema = z
  .strictObject({
    /** Adoption compares the exact locator. */
    locator: SessionLocatorSchema,
    /** Launch/adoption readback includes host PID. */
    hostPid: z.number().int().positive(),
    /** Launch/adoption readback includes the PID start token. */
    hostStartToken: z.string().min(1),
    /** Launch/adoption readback includes the process root. */
    processRoot: ProcessRootSchema,
    /** Launch/adoption readback includes the expected executable. */
    expectedExecutable: z.string().min(1),
    /** Launch/adoption compares the executable build. */
    executableBuildHash: z.string().min(1),
    /** Launch/adoption compares the engine build. */
    engineBuildId: z.string().min(1),
    /** Launch/adoption compares protocol major/minor. */
    protocol: SelectedProtocolSchema,
    /** Launch readback includes terminal geometry. */
    geometry: TerminalGeometrySchema,
    /** HostRecordV1 preserves exact lifecycle state. */
    state: z.enum(["starting", "live", "exited", "unknown"]),
    /** Launch/adoption compares the exclusive output sequence. */
    outputSeq: DecimalUint64Schema,
    /** HostRecordV1 preserves the newest checkpoint sequence. */
    checkpointSeq: DecimalUint64Schema,
    /** Launch readback proves the initial attaching visibility lease. */
    visibility: SessionVisibilitySchema,
  })
  .readonly();

/** HostRecordV1 is the complete strict on-disk recovery record. */
...
export const HostRegisterPayloadSchema = z
  .union([
    z.strictObject({
      /** Host launch registration is a v1 control payload. */
      schemaVersion: z.literal(1),
      /** Launch readback projects the listed HostRecordV1 identity/evidence fields. */
      record: HostRecordProjectionSchema,
    }),
    z.strictObject({
      /** Registration response is a v1 control payload. */
      schemaVersion: z.literal(1),
      /** Publication follows exact readback; shape adjudicated as accepted=true. */
...
export const HostAdoptPayloadSchema = z
  .union([
    z.strictObject({
      /** Broker adoption challenge is a v1 control payload. */
      schemaVersion: z.literal(1),
      /** terminal-stack-transition.html#security challenges with the 32-byte
       * secret; shape adjudicated as 64 lowercase hex. */
      adoptionSecretHex: Secret256HexSchema,
      /** Challenges the recorded exact locator. */
      expectedLocator: SessionLocatorSchema,
      /** The host validates the broker build. */
      brokerBuildId: z.string().min(1),
      /** The host validates broker protocol. */
      protocol: SelectedProtocolSchema,
      /** Only adoption is permitted on this challenge; shape adjudicated as a literal. */
      operation: z.literal("adopt"),
    }),
    z.strictObject({
      /** Adoption readback is a v1 control payload. */
      schemaVersion: z.literal(1),
      /** Adoption compares the exact locator. */
      locator: SessionLocatorSchema,
      /** Adoption compares host PID. */
      hostPid: z.number().int().positive(),
      /** Adoption compares the host PID start token. */
      hostStartToken: z.string().min(1),
      /** Adoption compares the live executable path. */
      executable: z.string().min(1),
      /** Adoption compares the executable build. */
      executableBuildHash: z.string().min(1),
      /** Adoption compares the engine build. */
      engineBuildId: z.string().min(1),
      /** Adoption compares the host-selected protocol, not the broker's offered constants. */
      protocol: SelectedProtocolSchema,
      /** Adoption compares the process root. */
      processRoot: ProcessRootSchema,
      /** Adoption compares the output sequence. */
      outputSeq: DecimalUint64Schema,
      /** Readback preserves checkpoint high-water evidence. */
      checkpointSeq: DecimalUint64Schema,
      /** Adoption verifies the current Workspace open-terminal revision. */
      visibility: SessionVisibilitySchema,
    }),
  ])
  .readonly();
...
export const GrantRegisterPayloadSchema = z
  .union([
    z.strictObject({
      /** One-use viewer grant registration is a v1 control payload. */
      schemaVersion: z.literal(1),
      /** terminal-stack-transition.html#visibility stores only the 256-bit hash;
       * shape adjudicated as sha256: plus lowercase hex. */
      grantTokenSha256: TaggedSha256Schema,
      /** terminal-stack-transition.html#surface issueAttach binds a grant to the
       * exact viewer identity. */
      viewerId: z.string().min(1),
      /** terminal-stack-transition.html#surface issueAttach binds the granted
       * operations. */
      operations: z.array(z.enum(["view", "human-input", "resize"])).readonly(),
      /** Unused attach grants expire after 30 seconds. */
      expiresAt: Rfc3339UtcMillisecondsSchema,
      /** terminal-stack-transition.html#surface issueAttach binds the requested
       * terminal geometry. */
      geometry: TerminalGeometrySchema,
    }),
    z.strictObject({
      /** Grant registration response is a v1 control payload. */
      schemaVersion: z.literal(1),
      /** Registration succeeds only after the host stores the hash; shape adjudicated true. */
      registered: z.literal(true),
    }),
  ])
  .readonly();

/** CREATE_BEGIN is the strict wire projection of SessionSpec. */
export const CreateBeginPayloadSchema = z
  .strictObject({
    ...SessionSpecSchema.unwrap().shape,
    /** Create consumes the daemon's just-revalidated pending open-terminal binding. */
  ...
  .readonly();
/** CREATE_COMMIT authenticates the bounded initial-input byte stream. */
export const CreateCommitPayloadSchema = z
  .strictObject({
    schemaVersion: z.literal(1),
  ...
  .readonly();
/** CREATED is the strict wire projection of CreateResult. */
export const CreatedPayloadSchema = z
  .strictObject({
    schemaVersion: z.literal(1),
...
export const TerminatedPayloadSchema = z
  ...
  .readonly();
/** VISIBILITY_RENEW combines the exact locator with VisibilityRequest. */
export const VisibilityRenewPayloadSchema = z
  ...
  .readonly();
/** RENEWED is the strict wire projection of VisibilityLease. */
export const RenewedPayloadSchema = z
  ...
  .readonly();
/** INPUT_ORPHAN_DISCARD: an authenticated delivery-time resolution of a
...
export const OrphanDiscardPayloadSchema = z
  ...
  .readonly();
/** ORPHAN_DISCARDED is deliberately a discriminated result: consumers
...
export const OrphanDiscardedPayloadSchema = z
  ...
  .readonly();
/** ATTACH_REQUEST combines the exact locator with AttachRequest. */
export const AttachRequestPayloadSchema = z
  ...
  .readonly();
/** ATTACH_GRANT is the strict wire projection of AttachGrant. */
export const AttachGrantPayloadSchema = z
  ...
  .readonly();
/** terminal-stack-transition.html#attach HOST_ATTACH presents the one-use token
...
// The transport-only payload schemas above have no SessionHost
...
type SessionSubjectSchemaMatchesContract = Assert<
  Equals<
    ...
    SessionHostContract.SessionSubject
  >
>;
type SessionLocatorSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof SessionLocatorSchema>,
    SessionHostContract.SessionLocator
  >
>;
type SessionSpecSchemaMatchesContract = Assert<
  Equals<z.infer<typeof SessionSpecSchema>, SessionHostContract.SessionSpec>
>;
type TerminalGeometrySchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalGeometrySchema>,
    SessionHostContract.TerminalGeometry
  >
>;
type SessionInspectionSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof SessionInspectionSchema>,
    SessionHostContract.SessionInspection
  >
>;
type CreateResultSchemaMatchesContract = Assert<
  Equals<z.infer<typeof CreateResultSchema>, SessionHostContract.CreateResult>
>;
type CaptureRequestSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof CaptureRequestSchema>,
    SessionHostContract.CaptureRequest
  >
>;
type CaptureResultSchemaMatchesContract = Assert<
  Equals<z.infer<typeof CaptureResultSchema>, SessionHostContract.CaptureResult>
>;
type AttachRequestSchemaMatchesContract = Assert<
  Equals<z.infer<typeof AttachRequestSchema>, SessionHostContract.AttachRequest>
>;
type AttachGrantSchemaMatchesContract = Assert<
  Equals<z.infer<typeof AttachGrantSchema>, SessionHostContract.AttachGrant>
>;
type VisibilityRequestSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof VisibilityRequestSchema>,
    SessionHostContract.VisibilityRequest
  >
>;
type VisibilityLeaseSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof VisibilityLeaseSchema>,
    SessionHostContract.VisibilityLease
  >
>;
type ResizeResultSchemaMatchesContract = Assert<
  Equals<z.infer<typeof ResizeResultSchema>, SessionHostContract.ResizeResult>
>;
type TerminalHostSessionRefSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostSessionRefSchema>,
    TerminalHostContract.SessionRef
  >
>;
type TerminalHostWindowSizeSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostWindowSizeSchema>,
    TerminalHostContract.WindowSize
  >
>;
type TerminalHostProcessIdentitySchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostProcessIdentitySchema>,
    TerminalHostContract.ProcessIdentity
  >
>;
type TerminalHostTerminalProfileSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostTerminalProfileSchema>,
    TerminalHostContract.TerminalProfile
  >
>;
type TerminalHostEnvironmentEntrySchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostEnvironmentEntrySchema>,
    TerminalHostContract.EnvironmentEntry
  >
>;
type TerminalHostTransferableHandleSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostTransferableHandleSchema>,
    TerminalHostContract.TransferableHandle
  >
>;
type TerminalHostDescriptorMappingSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostDescriptorMappingSchema>,
    TerminalHostContract.DescriptorMapping
  >
>;
type TerminalHostCommandSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostCommandSchema>,
    TerminalHostContract.Command
  >
>;
type TerminalHostLimitsSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostLimitsSchema>,
    TerminalHostContract.HostLimits
  >
>;
type TerminalHostJobControlEvidenceSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostJobControlEvidenceSchema>,
    TerminalHostContract.JobControlEvidence
  >
>;
type TerminalHostExitStatusSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostExitStatusSchema>,
    TerminalHostContract.ExitStatus
  >
>;
type TerminalHostReapEvidenceSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostReapEvidenceSchema>,
    TerminalHostContract.ReapEvidence
  >
>;
type TerminalHostLaunchOutcomeSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostLaunchOutcomeSchema>,
    TerminalHostContract.LaunchOutcome
  >
>;
type TerminalHostCreateRequestSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostCreateRequestSchema>,
    TerminalHostContract.CreateRequest
  >
>;
type TerminalHostCreateResultSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostCreateResultSchema>,
    TerminalHostContract.CreateResult
  >
>;
type TerminalHostInputClaimSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostInputClaimSchema>,
    TerminalHostContract.InputClaim
  >
>;
type TerminalHostClaimResultSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostClaimResultSchema>,
    TerminalHostContract.ClaimResult
  >
>;
type TerminalHostInputReceiptSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostInputReceiptSchema>,
    TerminalHostContract.InputReceipt
  >
>;
type TerminalHostResizeResultSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostResizeResultSchema>,
    TerminalHostContract.ResizeResult
  >
>;
type TerminalHostCheckpointSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostCheckpointSchema>,
    TerminalHostContract.Checkpoint
  >
>;
type TerminalHostSessionInspectionSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostSessionInspectionSchema>,
    TerminalHostContract.SessionInspection
  >
>;
type TerminalHostTerminationRequestSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostTerminationRequestSchema>,
    Parameters<TerminalHostContract.TerminalHost["terminate"]>[0]
  >
>;
type TerminalHostTerminationResultSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminalHostTerminationResultSchema>,
    TerminalHostContract.TerminationResult
  >
>;
type AutomatedInputSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof AutomatedInputSchema>,
    SessionHostContract.AutomatedInput
  >
>;
type InputReceiptSchemaMatchesContract = Assert<
  Equals<z.infer<typeof InputReceiptSchema>, SessionHostContract.InputReceipt>
>;
type TerminationRequestSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminationRequestSchema>,
    SessionHostContract.TerminationRequest
  >
>;
type TerminationResultSchemaMatchesContract = Assert<
  Equals<
    z.infer<typeof TerminationResultSchema>,
    SessionHostContract.TerminationResult
  >
>;
