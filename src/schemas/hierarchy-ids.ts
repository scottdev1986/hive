...
// Every revisioned record binds a content digest so a G1/G2 approval names exact bytes, never a floating "latest" pointer.
...
// The G1/G2 binding primitive: names one exact revision by its digest so the package a gate approves can never be re-described in free-form prose.
