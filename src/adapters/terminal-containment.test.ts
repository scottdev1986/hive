...
describe("terminal containment", () => {
  test("src cannot launch or track a terminal outside Workspace", async () => {
    ...
    for (const path of await sourceFiles(join(import.meta.dir, ".."))) {
      const source = await readFile(path, "utf8");
      for (const token of forbidden) {
        if (source.includes(token)) {
          violations.push(`${relative(join(import.meta.dir, ".."), path)}: ${token}`);
        }
      }
      ...
      if (externalOpen.test(source)) {
        violations.push(`${relative(join(import.meta.dir, ".."), path)}: external open -a`);
      }
    }
