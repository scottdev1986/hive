/** How a queen's skills reach her, which is a different question per vendor. An agent has a worktree, so its skills go in the worktree's native directory and every vendor reads them the same way. A queen runs in the user's own checkout, and Hive will not write her skills into it: that directory is read by the user's own sessions, and on three of the five vendors it is read by the queen whether Hive writes there or not. So each vendor is asked for an out-of-tree path instead, and each answers differently: - **claude** — `--plugin-dir`. The only mechanism that survives `--setting-sources user`, which the queen's launch passes to keep the repository's own settings out of her session and which switches off project skill discovery as a side effect. Plugin skills are namespaced, so hers arrive as `hive:<skill>`. - **kimi** — `--skills-dir`, which *replaces* user and project discovery rather than adding to it. That is the strongest isolation of the five: what Hive provisions is exactly what she has. - **grok** — `$GROK_HOME/skills`, under the redirected queen home. - **opencode** — `skills.paths` in Hive's generated configuration. - **codex** — nothing. `CODEX_HOME` is the only isolated path codex offers and auth lives inside it, so redirecting it would make Hive responsible for the queen's ability to log in. The Codex queen therefore reads the checkout's native skill directory and reports degraded isolation. */
