...
import { repairLegacyMountEvidence } from "../daemon/project-identity-core/project-identity-daemon";
...
/** A pre-fix updater executes the staged binary only once, with `--version`, before its own broken identity check blocks activation. Use that proof run as a narrow compatibility bridge: only a non-current native binary, and only a version probe, may refresh an existing record's mount-local evidence. */
