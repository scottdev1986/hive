...
import { HIVE_UPDATE_REPO } from "../shared/version";
...
/** Read a response body chunk by chunk rather than buffering it whole. `response.arrayBuffer()` cannot report progress because it resolves once, at the end. Streaming is what makes a bar possible at all, and it is also what lets us notice a truncated transfer at the moment the body ends rather than after the SHA-256 of a short file fails to explain itself. `Content-Length` is the server's claim about the body, not the manifest's. We report it as-is (null when absent) and let the caller reconcile it with the signed size; conflating the two here would hide the disagreement that matters. */
async function bytes(
  ...
  if (response.body === null) {
    // No stream to read (a mocked or bodiless response). Fall back rather than fail: the digest check downstream is what actually guards the bytes.
