/* Gate 6 (M1-B1) live proof — HEADLESS checkpoint authoring path. Adjudicated split (queen, 2026-07-18): checkpoint AUTHORING belongs to a headless libghostty-vt terminal (the host/sessiond side, which owns the ordered output stream); the embedded UI surface is a restore-only consumer (hive_ghostty_surface_restore_checkpoint_v1). This harness qualifies the authoring contract against the SHIPPED lib-vt artifact: export determinism, import round-trip, rejection of truncation / targeted header corruption (magic, version, engine-build-id binding, payload digest, body) / fuzzed garbage, null-argument safety, and no-poisoning after failed imports. Format facts (hive_checkpoint.zig): payload = "HVGCP001" magic(8) + version u16(2) + engine build id (self-hashing identity string starting at offset 10: pinned commit + zig toolchain shas + the patch sources themselves) + structural terminal/handler/pending state, bounded at max_payload_bytes (512 MiB; legacy allocating export stays at 64 MiB). decode() validates magic, version, build id, structural completeness (r.done()), and the size bound. It has NO body integrity digest BY DESIGN: at-rest/in-flight integrity is the HOST transport envelope's job (HVTCP001, which carries a payload sha256 — native/tests/abi/checkpoint-envelope.c), so a body bit-flip may import as different-but-well-formed state at this layer. That layering is part of the fork contract and is exercised as such below. */
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
/* Hive fork-contract symbols (native/include/hive_ghostty_bridge.h; signatures pinned there and by scripts/native/check-ghostty-abi.sh — declared locally because the bridge header pulls in the embedded-app ghostty.h, which the vt-only artifact deliberately does not ship). */
...
int main(int argc, char **argv) {
  ...
  /* Truncation: every shortened prefix is rejected, and a rejected import never poisons the terminal for a following valid one. */
  ...
  /* Targeted corruption at format-locked offsets: magic@0, version@8, ENGINE BUILD ID first byte @10 (build binding: the id is self-hashing over the pinned commit, zig toolchain, and the patch sources, so a different build can never import). */
  ...
  /* Body corruption: this layer deliberately carries no body digest (integrity = HVTCP001 host envelope), so a tail bit-flip may decode as different-but-well-formed state. The contract here is bounded behavior: no crash, and the terminal stays fully usable either way. */
  {
    uint8_t *mutant = malloc(len1);
  ...
  /* Fuzz-lite: deterministic garbage, half of it wearing a valid magic to get past the first gate. Import must reject everything, never crash. */
  for (int i = 0; i < 512; i++) {
  ...
  /* The cross-library release lock restores every corpus byte split into the real embedded surface. The manifest lets Swift prove that it is consuming this exact C-authored corpus rather than a duplicated fake. */
