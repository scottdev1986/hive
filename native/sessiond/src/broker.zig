...
const broker_host_client = @import("broker_host_client");
...
pub const ProcessStartToken = daemon_identity.ProcessStartToken;
pub const ObservedProcess = daemon_identity.ObservedProcess;
pub const formatStartToken = daemon_identity.formatStartToken;
pub const inspectProcess = daemon_identity.inspectProcess;
pub const ExactProcessPresence = daemon_identity.ExactProcessPresence;
pub const HostProcessOwnership = daemon_identity.HostProcessOwnership;
pub const observeExactProcess = daemon_identity.observeExactProcess;
pub const waitForExactProcessAbsence = daemon_identity.waitForExactProcessAbsence;
pub const ObservedPeer = daemon_identity.ObservedPeer;
pub const inspectPeer = daemon_identity.inspectPeer;
pub const ExpectedPeer = daemon_identity.ExpectedPeer;
pub const DaemonClaimChecks = daemon_identity.DaemonClaimChecks;
pub const VersionRange = daemon_identity.VersionRange;
pub const SessionProtocolRange = daemon_identity.SessionProtocolRange;
pub const DaemonControlIdentity = daemon_identity.DaemonControlIdentity;
pub const DaemonHello = daemon_identity.DaemonHello;
pub const DaemonHandshake = daemon_identity.DaemonHandshake;
pub const DaemonLock = daemon_identity.DaemonLock;
...
const frame_read_budget_ns = broker_transport.frame_read_budget_ns;
const FrameDeadlineReader = broker_transport.FrameDeadlineReader;
pub const writeRecordAtomic = broker_record.writeRecordAtomic;
pub const createAdoptionSecret = broker_record.createAdoptionSecret;
pub const Locator = broker_record.Locator;
pub const ProcessRoot = broker_record.ProcessRoot;
pub const Geometry = broker_record.Geometry;
pub const Visibility = broker_record.Visibility;
pub const HostRecord = broker_record.HostRecord;
const DiskLocator = broker_record.DiskLocator;
const DiskGeometry = broker_record.DiskGeometry;
const DiskHostRecord = broker_record.DiskHostRecord;
const locatorFromDisk = broker_record.locatorFromDisk;
const hostRecordFromDisk = broker_record.hostRecordFromDisk;
pub const SocketEvidence = broker_record.SocketEvidence;
const socketEvidenceAt = broker_record.socketEvidenceAt;
const socketEvidence = broker_record.socketEvidence;
const readAdoptionSecret = broker_record.readAdoptionSecret;
pub const verifySocket = broker_record.verifySocket;
const verifyPostConnectSocket = broker_record.verifyPostConnectSocket;
pub const recordJsonMatches = broker_record.recordJsonMatches;
pub const AdoptionReadback = broker_host_client.AdoptionReadback;
pub const TerminationReadback = broker_host_client.TerminationReadback;
pub const TerminationState = broker_host_client.TerminationState;
pub const TerminationCommand = broker_host_client.TerminationCommand;
pub const GrantRegistration = broker_host_client.GrantRegistration;
pub const HostRenewalResult = broker_host_client.HostRenewalResult;
pub const HostGrantResult = broker_host_client.HostGrantResult;
pub const HostControl = broker_host_client.HostControl;
const WireWelcome = broker_host_client.WireWelcome;
const locatorJsonValue = broker_host_client.locatorJsonValue;
pub const wallDeadline = broker_host_client.wallDeadline;
pub const WireHostClient = broker_host_client.WireHostClient;
pub const RecoveryChannel = broker_host_client.RecoveryChannel;
pub const RecoveryConnector = broker_host_client.RecoveryConnector;
pub const WireRecoveryConnector = broker_host_client.WireRecoveryConnector;
pub const verifyDaemonHello = daemon_identity.verifyDaemonHello;
pub const selectProtocolMinor = daemon_identity.selectProtocolMinor;
pub const parseDaemonHello = daemon_identity.parseDaemonHello;
pub const verifyDaemonPeer = daemon_identity.verifyDaemonPeer;
...
fn writeWelcome(
    ...
    const welcome = try buildWelcome(allocator, request, instance_id, build_id, server_epoch, selected_minor);
...
const test_daemon_lock_json =
    \\{"pid":42,"instanceId":"instance-a","startedAt":"2026-07-18T00:00:00Z","startToken":"10:2","executablePath":"/opt/hive/bin/hive"}
;

const test_daemon_handshake_json =
    \\{"productVersion":"0.0.0-dev","buildHash":"daemon-build","wireProtocol":{"min":1,"max":1},"schemaEpoch":1,"capabilities":["daemon-handshake-v1"],"instanceId":"instance-a","hiveUuid":"hive-a","identityKey":"project-a","repoFamilyKey":"family-a","generation":1}
;

const test_daemon_handshake_mismatch_json =
    \\{"productVersion":"0.0.0-dev","buildHash":"daemon-build","wireProtocol":{"min":1,"max":1},"schemaEpoch":1,"capabilities":["daemon-handshake-v1"],"instanceId":"instance-b","hiveUuid":"hive-a","identityKey":"project-a","repoFamilyKey":"family-a","generation":1}
;

const TestHandshakeServer = struct {
    listener: std.net.Server,
    body: []const u8,

    fn run(self: *TestHandshakeServer) void {
...
fn testObservedDaemonPeer() ObservedPeer {
    const executable = "/opt/hive/bin/hive";
    var observed: ObservedPeer = .{
        .uid = std.posix.getuid(),
        .gid = @intCast(c.getgid()),
        .pid = 42,
    ...
    @memcpy(observed.executable[0..executable.len], executable);
    return observed;
}
...
test "pre-WELCOME evidence failure sends a correlated typed ERROR" {
...
test "WELCOME validation failure sends a typed ERROR with no WELCOME bytes" {
    ...
    var timer = try std.time.Timer.start();
    // An empty broker build id fails welcomePayload validation before any
    // WELCOME byte is written, so the client must see the typed ERROR as the
...
test "WELCOME transport failure closes bare with no ERROR frame" {
    ...
    var client: std.net.Stream = .{ .handle = sockets[0] };
    defer client.close();
    // Shutting down the broker's write side before it serves makes the WELCOME
