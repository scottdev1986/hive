...
/// Bound on parent wait for the exec-barrier (R5). Wedged pre-exec children fail at this bound rather than hanging the daemon spawn path forever.
pub const exec_barrier_timeout_ms: c_int = 5_000;

/// Bound on the reap wait after SIGKILL (matches the 2 s KILL settle bound). A child wedged in uninterruptible I/O fails at this bound instead of hanging the spawn/deinit path on a blocking waitpid forever.
...
pub const PtyHost = struct {
    ...
    pub fn spawn(self: *PtyHost, spec: SpawnSpec) Error!SpawnOutcome {
        ...
        // B1: errdefer skips fds set to -1 so manual close + errdefer never double-close a recycled descriptor number in multithreaded sessiond.
        errdefer {
        ...
        // Keep the barrier above every declared target so descriptor transfer cannot overwrite it in the child.
        ...
        if (pid == 0) {
            ...
            if (c.ioctl(slave, c.TIOCSWINSZ, &ws) != 0)
                childBarrierFail(exec_pipe[1], .terminal_setup, 126);
            // R2: every pre-exec failure reports through the barrier — never bare _exit, which looks like EOF/success to the parent.
        ...
        // R1/R3/R5: barrier wait — poll with timeout; read retries EINTR. EOF (0) ⇒ exec succeeded; sizeof(c_int) ⇒ child reported failure.
        ...
        // Identity tuple from process_inspector — record whatever proc_pidpath returns WITHOUT gating liveness on a basename match (shebang shims resolve to node/sh/etc.).
    ...
    /// The ordered position the CURRENT revision was applied at. Retained separately from `operation_sequence`, which later input advances: a receipt reconstructed from the live sequence would report a position the resize never occupied.
    ...
    /// Atomically accept one contiguous write range into the ordered queue. Either the whole range is queued or nothing is (no partial acceptance).
    ...
    /// Drain the ordered write queue to the PTY master. Short writes resume in order; never reorders. Returns bytes written this call.
    ...
    pub fn readAvailable(self: *PtyHost) Error!ReadChunk {
        ...
        const n = posix.read(self.master_fd, self.read_buf) catch |err| switch (err) {
            ...
            error.BrokenPipe, error.ConnectionResetByPeer => return error.Closed,
            // Darwin hangup: a master read after the last slave close returns EIO (xnu pts master). That is peer-gone, not an I/O fault report the dead session cleanly as Closed.
    ...
    /// A sibling authority performed the direct-parent waitpid for this exact child. Forget the pid before deinit so it can never target a reused pid.
    ...
    pub fn waitExit(self: *PtyHost, hang: bool) Error!ExitEvidence {
        ...
        if (rc < 0) {
            if (std.c._errno().* == @intFromEnum(std.posix.E.CHILD)) {
                // Another authority consumed the status. Forget the pid before it can be reused;
                // the evidence stays unknown because this parent did not observe the exit.
                self.pid = -1;
                return .{
    ...
    fn forceKillChild(self: *PtyHost) void {
        if (self.pid > 0) {
            // The start token exists only after post-exec identity observation; earlier spawn-failure paths have no token to verify against (the just-forked child is still ours and unreaped, so its pid cannot have been reused).
    ...
    /// SIGKILL our own child, then reap with a bounded wait. When a start token is on record, re-observe and verify it first ( TOCTOU): if the recorded identity is gone — reaped by a sibling authority and the pid since reused — the pid now belongs to an unrelated process and must NOT be signalled. Unobservable fails closed (skip) because reuse cannot be ruled out. Returns true when the child was reaped or positively absent; false when the signal was skipped or the reap deadline expired — the caller must forget the pid rather than hang on a child wedged in uninterruptible I/O.
    fn forceKillChildWithPid(
        ...
        // Bounded reap: a blocking waitpid hangs forever on a child wedged in uninterruptible I/O and would stall the spawn/deinit path.
        var st: c_int = 0;
        var waited_ms: u32 = 0;
...
/// Parent-side barrier: poll with bound (R5), read with EINTR retry (R3). EOF ⇒ success; a complete typed evidence frame ⇒ exec_failed. deadline is monotonic — EINTR must not reset the full timeout budget (SIGCHLD from other children is common in sessiond).
...
fn childBarrierFail(
    ...
    while (sent < bytes.len) {
        ...
        if (n < 0) {
            ...
            // L1: a blocking write with the parent still holding the read end can only fail with EINTR in practice (which we retry). EPIPE needs the parent's read end closed; EAGAIN needs O_NONBLOCK. This break is defensive/unreachable under our fd setup — then _exit closes the write end and the parent would see EOF/.success. We still _exit (must not hang the child); do not claim the parent times out.
            break;
        }
        if (n == 0) break;
        sent += @intCast(n);
...
test "writeDrainAll bounds a stalled consumer with DrainStalled and keeps the queue" {
    ...
    // Bytes are not dropped: the undrained tail stays queued and the host loop's writeDrain keeps working after the stall.
...
/// Reads one chunk into `got`, sleeping briefly when the read comes back empty. Returns false when the PTY closed.
fn appendAvailableChunk(host: *PtyHost, got: *std.ArrayList(u8)) !bool {
...
test "DROP-A-CHUNK canary: harness FAILS when a chunk is dropped" {
    ...
    // Positive control for the digest harness: if production dropped a chunk, the comparison must fail. This test deliberately drops chunk 0 and asserts digests differ — proving the canary can see a bug.
    var tmp_dir = testing.tmpDir(.{});
    defer tmp_dir.cleanup();
...
// Exec-barrier: nonexistent binary → child writes typed errno evidence. Must NOT return a SpawnReadback claiming a live child (acting≠being).
...
// R2 positive control: bad cwd → chdir fails → barrier reports failure. Old bare _exit(125) looked like EOF/success and returned a live zombie readback.
...
// R1 positive control: shebang/shim whose RESOLVED image ≠ argv[0] basename must still SUCCEED. Old basename-match code wrongly SIGKILLed these.
...
// PID-reuse guard positive control: the force-kill path must NOT signal a pid whose recorded start token no longer matches (reaped by a sibling authority and the pid since reused). Unfixed code SIGKILLs the bare pid — this test FAILS if the real child is killed after its recorded token is replaced. (forceKillChild is exercised directly: deinit's closeMaster would SIGHUP the session on its own and drown the signal this test isolates.)
...
// Matching-token path: deinit SIGKILLs our own child and the bounded reap produces positive absence evidence (the reap bound must not break the ordinary force-kill path).
...
// Darwin hangup: after the child exits (last slave close), a master read returns EIO — must surface as Closed, not IoFailed, so dead sessions are reported cleanly.
