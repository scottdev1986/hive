...
## 9. Defect fixes (2026-07-22, user-reported against the deployed daemon)

- **D1 — semantic layer dead in the compiled daemon** (`61ef5c11`): single-file `bun build --compile` cannot carry onnxruntime; compiled binaries also cannot resolve node_modules packages from external paths (spike-proven). Fix: fastembed is pre-bundled into a single ESM file under `~/.hive/tools/embeddings/` (resolution chain `HIVE_EMBEDDINGS_HOME` → tools dir → repo node_modules), provisioned by `hive embeddings install` with a probe-verified install (dimensions asserted). Distinct labeled failure states: runtime-missing / runtime-broken / native-unloadable. Deployed-binary end-to-end proof: 3 articles written through the live daemon → 3 vector rows; zero-overlap paraphrase recall hit.
- **D2 — silent degradation** (`49c99cf2`): `memory_recall` envelope gains `semantic: hybrid | disabled | degraded:<state>` with an unclamplable ⚠ warning line; `memory_write`/`memory_note` report `embedding: indexed|queued|unavailable:<state>`; daemon warnings persist to `~/.hive/logs/daemon.log` (5 MB rollover); `hive_status` exposes `memory.embeddings` (state, vector counts, runtimeDir).
- **D3 — health check proved the wrong surface** (`f80c6ad2`): `hive memory self-test --live` drives the RUNNING daemon over MCP (10 assertions: discovery, reported state, write projections, paraphrase recall asserting `semantic: hybrid`, FTS/read-back, cleanup). The degraded-daemon FAIL path is test-proven — this is the regression guard for D1.
- **D4 — semantic assertions opt-in in CI** (`8e8d773c`): `--strict` turns any SKIP into a FAIL on both tiers; `release.yml` (the workflow that runs `bun test` on every push to main) now caches the model and runs `HIVE_MEMORY_SELF_TEST_EMBEDDINGS=1 ... self-test --strict`, so an embedding regression blocks main.

## 10. Deployment, packaging, and dev/prod memory sharing (2026-07-22)

- **Dev flow** (`debc887e`): `make run` provisions the embedding runtime and REFUSES to start a degraded dev daemon (D1 guard); `verify-dev-run.ts` asserts `memory.embeddings state=ready` at startup. Dev HIVE_HOME symlinks `memory/`, `projects/`, `project-registry.json`, `models/` to the real `~/.hive` (user ruling: dev testing must see lessons learned in live, and vice versa); runtime state stays per-home; `clean` gained `-L` refuse-guards and the traversal audit is recorded in the Makefile. Pre-existing dev homes with real per-home memory dirs are never merged or deleted — loud manual-reconcile warning.
- **Self-contained release** (`601d627d`): `dist/embeddings-runtime.tar.gz` is a first-class release artifact (manifest SHA-256 + Ed25519 like every artifact; `scripts/signing/verify.sh` gates its layout). `hive embeddings install` resolves: explicit `--from`/checkout copy (dev) → verified release download keyed to the binary's own version (prod). `hive init` auto-provisions by default (`--no-embeddings` opt-out; offline → honest deferred state, init still completes). Compatibility note: the new `"embeddings"` manifest kind makes pre-this-release binaries refuse new manifests in `hive update` (same trade as `adc5df1b`).

## 11. Mandatory embeddings + README (user ruling 2026-07-22)

Memory and embeddings are NOT optional and carry no opt-out (`662b09b1`): the `--no-embeddings` flag is removed — `hive init` always provisions, and `hive update` re-provisions the runtime pinned to the newly activated version (skip-fast when healthy). Install/update failure is a loud alarm (`⚠ EMBEDDINGS NOT INSTALLED — memory is DEGRADED … run hive embeddings install`), never a silent state and never a user choice. `hive embeddings install` is now documented as the repair path for a failed/deferred install. README.md documents the memory system (`3a62f455`): layers, summoned recall, commands, config, isolation — verified against src/cli.ts and src/schemas/config.ts.
