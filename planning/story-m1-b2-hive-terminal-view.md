...
Delivery is split without weakening that gate. **B2.1a** wires authenticated create/sustain visibility and explicitly disclaims close/quit completion. **B2.1b** completes the lifecycle with locator-fenced pane close, authoritative provider-tree readback before daemon shutdown, terminate-later Workspace quit, publisher-death expiry, and visible failure. B2.2 depends on both halves; the visibility lease is only the crash backstop, never the ordinary close/quit mechanism.
...
| B2.1a · Visibility freeze and create/sustain source | B2.0 | Freeze visibility request/lease behavior through the reviewed A0 contract gate; build the authenticated full-snapshot channel, live PID/start-token/revision validation, pending-pane create gate, renewal/re-attestation, and fail-closed replay/PID-reuse rules. | Contract fixtures prove the freeze; a neutral process is created only from a visible pending pane and stale/spoofed sources fail. Close/quit completion is explicitly deferred to B2.1b. | Claude → Grok |
| B2.1b · Exact close and quit teardown | B2.1a | Fence pane kill by the held exact locator; terminate every sessiond tree with authoritative absence readback before daemon shutdown; hold AppKit termination until success; retain publisher-death expiry only as the crash backstop. | Stale locator kills nothing; explicit close and clean quit prove exact provider-tree absence; teardown failure remains visible; publisher death independently proves bounded expiry. | Codex → cross-vendor review |
| B2.2 · Attach/output/reconnect | B2.1b | Exact grant/binding, manual output ingestion, checkpoint/replay, acknowledgements, first-correct-frame, retarget fencing, typed gaps/failures, exit/reap. | Live neutral TUI survives daemon restart and renderer recreation at adversarial byte splits; digests/high-water match; wrong build/gap/late generation fail closed. | Grok → Codex |
