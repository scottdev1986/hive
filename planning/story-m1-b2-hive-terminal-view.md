...
1. **M1-A2 is the backend prerequisite.** Its exact-generation create, inspect, list, terminate, byte I/O, resize, attach/replay, exit, and reap postconditions remain authoritative. B2 consumes that neutral behavior; it does not add Hive, vendor, repository, or Workspace policy below the neutral boundary.
2. **M1-B1 is the engine prerequisite.** B2 inherits the B1-qualified upstream Ghostty commit, Hive patch series, toolchain, build identity, ABI manifest, and behavior corpus. B2 may not silently move the pin or weaken a B1 gate.
3. **The Workspace-visibility interlock lands first inside B2.** Production create admission is intentionally closed until a real Workspace pane supplies a live Workspace PID, its exact start token, and an open-terminal inventory revision. This is its own build increment, not a separate story and not part of renderer transport.
4. **Renderer integration follows the visibility source.** A visible starting/attaching pane record exists before create; the created exact generation then attaches to its manual-I/O surface. Attach transport may be replaced without changing logical visibility.
5. **No removal happens in B2.** SwiftTerm and the tmux attach path stay frozen while B2 qualifies. When B2 is green, STORY-001 and STORY-002 execute as one atomic hard cut with no renderer or host fallback. Their full matrix is re-run on the post-deletion tree before that cut lands.
6. **M1 remains provider-policy neutral.** Real vendor binaries are manually launched as black-box terminal probes. Provider launch profiles, readiness, belief, approvals, messaging, and authenticated status remain M2 work.
...
This increment is project-neutral at the host boundary: Workspace policy terminates at the daemon adapter, while sessiond receives only exact identity and visibility evidence already defined by M1-A2.
...
- ECMA-48 control parsing at every byte boundary plus xterm primary/alternate screen 1049, bracketed paste 2004, focus 1004, mouse 9/1000/1002/1003/1005/1006/1007/1015/1016, cursor shape, title/bell, OSC 8 hyperlinks, denied OSC 52, 256-color, truecolor, synchronized output, and query/reply fixtures.
...
| B2.1 · Workspace visibility source | B2.0 | Authenticated full-snapshot inventory channel; live PID/start-token/revision validation; pending-pane create gate; renew/remove/re-attest; fail-closed replay/PID-reuse rules. | A neutral full-screen process can be created only from a visible pending pane; stale/spoofed sources fail; pane close, quit, and publisher death yield verified tree absence. | Claude → Grok |
...
| B2.3 · Input/geometry | B2.2 | First responder and claims; Ghostty key/text/preedit/paste/mouse encoders; `INPUT_SUBMIT`/`APPLIED`; measured resize and final SIGWINCH; receipt/error UI. | Byte-capture fixture plus vttest proves keys, Kitty modes, dead key, CJK IME, bracketed paste boundaries, mouse modes, Retina resize, retry/unknown behavior, and no double input. | Codex → Claude |
| B2.4 · Viewer semantics and accessibility | B2.3 | Bounded scrollback/search, selection/copy, local-vs-captured mouse, lifecycle states, GPU/occlusion/sleep behavior, AppKit accessibility tree/notifications. | Pinned vttest/VT corpus passes; Instruments bounds hold; recorded VoiceOver and Accessibility Inspector acceptance is reproduced by the reviewer. | Claude → Grok |
...
None requiring queen/user adjudication were found. The potentially ambiguous seams are resolved here: B1's existing Ghostty pin is retained; sessiond is the single terminal-query-reply authority; human renderer bytes use `INPUT_SUBMIT` rather than the raw streaming optimization; oversize atomic human input fails visibly at the adopted v1 cap; and Workspace visibility is a separate B2 increment/channel whose authority comes from the live pane inventory rather than renderer attachment.
...
2. A real Workspace pane supplies fresh PID/start-token/revision visibility, opens production sessiond create admission only for an exact visible pending record, renews only from the current inventory, and tears down on pane/window/Workspace close or bounded visibility expiry.
...
10. The evidence bundle is accepted as the renderer input to the STORY-001/STORY-002 Removal Gate. No SwiftTerm/tmux removal, legacy shim, dual-renderer flag, or fallback is introduced by B2; the full matrix remains mandatory again on the atomic deletion tree.
...
- **STORY-002 — Complete removal of agent TUI code:** Scope; Sequencing; DoD-2 live proof across Claude Code, Codex, and Grok; DoD-3 fidelity floor; DoD-3a live accessibility; External documentation.
...
- Changing the neutral M1-A2 host contract, the adopted M1-A1 input wire, the B1 Ghostty pin/patch budget, or inventing a stable libghostty ABI. A required contract change returns to its owning story and review gate.
