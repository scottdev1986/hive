...
- **Where the cut lands (user ruling Q2, RATIFIED final):** the rebuild proceeds **on main progressively**; a checksummed known-good OLD Hive release is pinned as an **isolated bootstrap tool** (own HIVE_HOME/DB/socket/ports, provably no access to new runtime state — story M1-BOOT) and orchestrates development while main carries the new/gutted state. No long-lived integration branch; branches only for work organization. Release/activation to consumers is a **separate END gate** after the whole rebuild is done AND stable. The Removal Gate still governs this cut: main is never gutted before the replacement qualifies. The pre-cut drain (below) is executed BY the bootstrap instance.
