...
**Not available at all in the pin, and therefore not specifiable:** cursor-aware ligature suppression. The elegant treatment — render ligatures while reading, dissolve them when the cursor sits inside one — exists in another terminal, not in this engine. This engine exposes ligature control only as a static OpenType feature toggle. The design uses the static toggle and does not pretend otherwise.
...
**Grid font: the JetBrains Mono Nerd Font already embedded in the pinned engine, at 13 pt, with a fallback chain of Menlo then Monaco.**

Rationale, in order of how load-bearing each point is:

- **SF Mono is rejected, on two independent grounds, and this must not be re-proposed in review.** Legally, its license permits use solely in conjunction with Apple-branded applications and forbids embedding or use in other software; Hive is not an Apple-branded application. Technically — and this is decisive regardless of the license — SF Mono is not resolvable by name: requests for it return nothing, and a request for the internal system-monospaced name silently yields a serif fallback with a system warning. The system monospaced face is reachable only through a dedicated AppKit API that returns an API-only font object with no public name. The engine resolves font families **by name**. There is therefore no name to give it, and a "System Monospaced" option in the terminal grid is not buildable on this engine at any price. (The API remains available for AppKit chrome, which is a separate decision below.)
- **The default costs nothing to adopt.** The Nerd Font-patched JetBrains Mono ships inside the pinned engine in regular, bold, italic, and bold-italic, under the SIL Open Font License. No new dependency, no new license review, and the Nerd Font patching supplies the powerline and status glyphs that the vendor TUIs actually draw with — which matters because those TUIs are the content.
...
- **Fallbacks are name-resolvable and preinstalled.** Menlo is the system's own fixed-pitch default; Monaco backs it. Both are present on every macOS install. The engine's font-family setting is repeatable, so the chain is expressed directly.
...
It rests on a terminal-specific argument that is sound: a ligated `!=` becomes visually indistinguishable from the real U+2260 codepoint, and **the user's entire job in a terminal is knowing exactly which bytes are on screen.** Ligature substitution is also context-blind — it considers character order only — so it is occasionally semantically wrong. Another terminal's own documentation concedes the narrower point that ligatures are good for code editing but not for other terminal applications, because a multi-cell glyph fights a fixed-cell grid. Since the cursor-aware compromise is unavailable in this pin, off is the correct static default for a byte-exact viewer.
...
| `bold-color` | Explicit. Note this option absorbed the older bold-is-bright setting in a prior engine release; the old name does not exist in this pin and must not be specified. |
...
| C1.1 · Typography and cell metrics | C1.0 | Grid font chain, size, ligature suppression, weight rules, thickening as flag-plus-strength, cell-height adjustment. | The embedded font resolves with the documented fallback chain on a machine with no extra fonts installed; a byte-exactness check confirms no ligature substitution; powerline and status glyphs from all three vendor TUIs align at the chosen cell height; thickening at strength zero is proven distinct from thickening disabled. | Claude → Grok |
...
2. The grid renders in the engine-embedded open-licensed monospace face at the system default size with a documented, name-resolvable fallback chain, verified on a machine carrying no additional fonts. SF Mono is not used, not referenced, and not shippable; the rejection rationale is recorded so it is not re-proposed.
3. Ligature substitution is off and byte-exactness is demonstrated. Weight, tracking, thickening, and cell-height decisions are implemented as specified, with thickening exposed as an enable flag plus a strength because strength zero is not "off."
...
- Cursor-aware ligature suppression, which the pin cannot express.
